...
type
  TDMPropuestasConfirm = class(TDataModule)
     ...
     procedure DMDocsPropConfProvCreate(Sender: TObject);
...
var
  DMPropuestasConfirm : TDMPropuestasConfirm;
  Serie : string;

implementation

uses UDMMain, UEntorno, UUtiles, UDameDato, UFMPropuestasConfirm, URecursos,
  UFBusca, UFMUnidadesExtGesC, UFInfoHistorico, UFMPreciosArticuloDoc, UFParada,
  UFMPedidosAProveedor, UFOrdenaLineas, UFMain, Math, UParam;

{$R *.DFM}

procedure TDMPropuestasConfirm.DMDocsPropConfProvCreate(Sender: TObject);
//var
//i : integer;
begin
  ...
  Kriconf467 := (DMMain.EstadoKri(467) = 1);
  Param_SYSCONF005 := StrToIntDef(LeeParametro('SYSCONF005'), 0);
  Param_DOCREFE006 := (LeeParametro('DOCREFE006') = 'S');
  ...
  begin
     Clear;
     Add(' SELECT * FROM VER_CABECERAS_OCP ');
     Add(' WHERE ');
...
procedure TDMPropuestasConfirm.Graba(DataSet: TDataSet);
begin
...
procedure TDMPropuestasConfirm.QMDetalleNewRecord(DataSet: TDataSet);
var
  i : integer;
...
procedure TDMPropuestasConfirm.QMCabeceraNewRecord(DataSet: TDataSet);
begin
  ...
  QMCabeceraALMACEN.AsString := DMMain.DameAlmacenDocumento('OCP', Serie);
...
procedure TDMPropuestasConfirm.QMCabeceraBeforePost(DataSet: TDataSet);
var
  Documentos : string;
begin
  QMCabeceraENTRADA.AsInteger := REntorno.Entrada;

  if Param_DOCREFE006 and (Trim(QMCabeceraSU_REFERENCIA.AsString) <> '') then
  begin
     ...
     begin
        try
           ...
           try
              ...
              SelectSQL.Add(' JOIN GES_CABECERAS_E_OCP D ON C.ID_E = D.ID_E ');
              SelectSQL.Add(' WHERE ');
...
procedure TDMPropuestasConfirm.QMDetalleARTICULOChange(Sender: TField);
var
  Baja : boolean;
begin
  ...
  if (not DMMain.ArticuloBloqueado(Sender.AsString, 'OCP')) then
  begin
     xArticulos.Close;
     xArticulos.Open;
...
procedure TDMPropuestasConfirm.InfoActualiza;
begin
...
procedure TDMPropuestasConfirm.QMDetalleBeforePost(DataSet: TDataSet);
var
  Division : double;
begin
  if (DataSet.State = dsInsert) then
...
procedure TDMPropuestasConfirm.QMCabeceraAfterOpen(DataSet: TDataSet);
begin
  xProveedor.Open;
  xDirecciones.Open;
  QMDetalle.Open;
...
procedure TDMPropuestasConfirm.QMCabeceraALMACENChange(Sender: TField);
begin
...
procedure TDMPropuestasConfirm.AntesDeEditar(DataSet: TDataSet);
begin
...
procedure TDMPropuestasConfirm.QMCabeceraPROVEEDORChange(Sender: TField);
begin
...
procedure TDMPropuestasConfirm.Duplica;
var
...
begin
  ...
  begin
     try
        ...
        SQL.Add(' (?EMPRESA, ?EJERCICIO, ?CANAL, ?SERIE, ''OCP'', ?RIG, ');
...
function TDMPropuestasConfirm.BusquedaCompleja: integer;
begin
  with QMCabecera do
  begin
...
procedure TDMPropuestasConfirm.QMCabeceraAfterPost(DataSet: TDataSet);
begin
  Graba(DataSet);
  {
...
procedure TDMPropuestasConfirm.QMCabeceraPostError(DataSet: TDataSet; E: EDatabaseError; var Action: TDataAction);
begin
...
procedure TDMPropuestasConfirm.CambiaSerie(SerieVar: string);
begin
  Serie := SerieVar;
  with QMCabecera do
  begin
     ...
     if (LeeParametro('OCPULTD001', Serie) = 'S') then
        Last;
  end;
...
procedure TDMPropuestasConfirm.SerieChange(SerieVar: string);
begin
  with xSeries do
  begin
     Close;
...
procedure TDMPropuestasConfirm.ActualizaCondicionesProv(CrearCondicionModelo: boolean);
begin
...
procedure TDMPropuestasConfirm.CambiaTarifa(Tarifa: string);
begin
...
procedure TDMPropuestasConfirm.InsertaArticulo(Articulo: string);
begin
  FMPropuestasConfirm.DBGFDetalle.SetFocus;
  QMDetalle.Append;
  QMDetalleARTICULO.AsString := Articulo;
end;

procedure TDMPropuestasConfirm.xArticulosTarifaAfterOpen(DataSet: TDataSet);
begin
  xtarifasProveedor.Open;
end;

procedure TDMPropuestasConfirm.CambiaUnidadesExt(Articulo: TField; Modo: boolean);
var
...
procedure TDMPropuestasConfirm.CambiaPreciosTarifa;
var
  bueno : boolean;
begin
...
procedure TDMPropuestasConfirm.QMCabeceraFORMA_DE_PAGOChange(Sender: TField);
begin
  with xFormasPago do
  begin
     Close;
...
procedure TDMPropuestasConfirm.BorraLineaExt;
begin
  if QMDetalle.State = dsInsert then
...
procedure TDMPropuestasConfirm.DatosStocks(var Articulo, Almacen: string);
begin
  Articulo := QMDetalleARTICULO.AsString;
  Almacen := QMCabeceraALMACEN.AsString;
end;

procedure TDMPropuestasConfirm.DatosHerencia(var rig, ejercicio: integer; var tipo, num_factura, titulo, serie: string);
begin
...
procedure TDMPropuestasConfirm.PreparaNotasDetalle;
begin
  if (QMDetalle.State in [dsEdit, dsInsert]) then
     QMDetalle.Post;
...
function TDMPropuestasConfirm.Traspasado: boolean;
var
  Q : THYFIBQuery;
begin
...
procedure TDMPropuestasConfirm.RefrescaTabla;
begin
...
// Histórico de Stocks
procedure TDMPropuestasConfirm.Historico;
begin
...
procedure TDMPropuestasConfirm.AnulaDocumento;
begin
...
procedure TDMPropuestasConfirm.MascarasMoneda;
begin
...
procedure TDMPropuestasConfirm.CambiaMonedaDetalle;
begin
...
procedure TDMPropuestasConfirm.QMCabeceraMONEDAChange(Sender: TField);
begin
  xMonedas.Close;
  xMonedas.Open;
end;

procedure TDMPropuestasConfirm.QMCabeceraTOTAL_CANALGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := FormatFloat(DMMain.MascaraMoneda(QMCabeceraMONEDA.AsString, 1),
     QMCabeceraLIQUIDO.AsFloat);
end;

procedure TDMPropuestasConfirm.FiltraCabecera(Ejercicio: integer; Serie: string; Rig: integer);
begin
  with QMCabecera do
  begin
     Close;
     ...
     Open;
     FMPropuestasConfirm.EFSerie.Text := Serie;
  end;
end;

function TDMPropuestasConfirm.HayDocumentos: boolean;
begin
  Result := QMCabecera.Active and (QMCabecera.FieldByName('RIG').AsInteger <> 0);
end;

function TDMPropuestasConfirm.EstadoDocumento: integer;
begin
  Result := -1;
...
function TDMPropuestasConfirm.HayDetalle: boolean;
begin
  //Result := (QMDetalle.FieldByName('LINEA').AsInteger <> 0);
  Result := QMDetalle.Active and (QMDetalle.RecordCount > 0);
end;

procedure TDMPropuestasConfirm.QMCabeceraAfterInsert(DataSet: TDataSet);
begin
  MonedaOld := QMCabeceraMONEDA.AsString;
end;

procedure TDMPropuestasConfirm.QMCabeceraFECHAChange(Sender: TField);
begin
  with xProveedor do
  begin
     Close;
...
procedure TDMPropuestasConfirm.QMDetalleUNIDADESChange(Sender: TField);
begin
...
procedure TDMPropuestasConfirm.ControlaRestriccion;
begin
  ControlaBorrado(QMCabeceraID_E.AsInteger, QMCabeceraTIPO.AsString);
end;

procedure TDMPropuestasConfirm.CambiaMonedaFecha;
var
  Q : THYFIBQuery;
begin
  Q := THYFIBQuery.Create(nil);
...
procedure TDMPropuestasConfirm.QMCabeceraAfterScroll(DataSet: TDataSet);
begin
  Fecha_old := QMCabeceraFECHA.AsDateTime;
end;

procedure TDMPropuestasConfirm.OrdenarLineas;
begin
  QMDetalle.DisableControls;
  OrdenaLineas('OCP', QMCabeceraID_E.AsInteger, Self);
  QMDetalle.Close;
  QMDetalle.Open;
  QMDetalle.EnableControls;
end;

procedure TDMPropuestasConfirm.TraspasarA(Tipo_Destino: string);
var
  id_e_d : integer;
begin
...
procedure TDMPropuestasConfirm.AbreDocumentos(id_e_d: integer; tipo_destino: string);
begin
  if (tipo_destino = 'PEP') then
     TraspasoPedidoProv(id_e_d);
end;

procedure TDMPropuestasConfirm.RefrescaCabecera(Accion: integer);
var
  Marca : TBookmark;
begin
  case Accion of
...
procedure TDMPropuestasConfirm.QMCabeceraCAMBIO_FIJOChange(Sender: TField);
begin
  QMCabeceraVALOR_CAMB_FIJO.AsFloat := 0;
end;

procedure TDMPropuestasConfirm.QMCabeceraPROYECTOChange(Sender: TField);
var
...
procedure TDMPropuestasConfirm.BuscaArticulo(Articulo: string);
begin
  DMMain.LogIni('DMPropuestasConfirm.BuscaArticulo');
...
function TDMPropuestasConfirm.Inhabilitar: boolean;
begin
  Result := ((QMDetalle.State = dsInsert) or (QMDetalle.State = dsEdit));
end;

procedure TDMPropuestasConfirm.DataModuleDestroy(Sender: TObject);
begin
...
procedure TDMPropuestasConfirm.BusquedaArticulo(Descripcion: string);
var
...
procedure TDMPropuestasConfirm.QMDetalleAfterOpen(DataSet: TDataSet);
begin
  RellenaDatosLinea(OldLinea);
end;

procedure TDMPropuestasConfirm.QMDetalleAfterScroll(DataSet: TDataSet);
begin
  RellenaDatosLinea(OldLinea);
end;

procedure TDMPropuestasConfirm.RellenaDatosOldLinea;
begin
  RellenaDatosLinea(OldLinea);
end;

procedure TDMPropuestasConfirm.RellenaDatosLinea(Linea: TStrings);
...
procedure TDMPropuestasConfirm.xInfoActualizadaTITULO_ESTADOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := DameTituloEstado(xInfoActualizadaESTADO.AsInteger);
end;

procedure TDMPropuestasConfirm.DuplicarLinea;
var
  old_KriConf467 : boolean;
  // Q : THYFIBQuery;
  // IdDetallesE, i, NuevaLinea : integer;
begin
  old_KriConf467 := KriConf467;
  try
     KriConf467 := True;
     // IdDetallesE := QMDetalleID_DETALLES_E.AsInteger;

     // Guardo los datos con lo que hay en la linea si KriConf467 estaba deshabilidatdo
     ...
     KriConf467 := old_KriConf467;
  end;
end;

procedure TDMPropuestasConfirm.QMDetalleTIPO_UNIDAD_LOGISTICAChange(Sender: TField);
begin
  if ((QMDetalle.State = dsInsert) or (QMDetalle.State = dsEdit)) and (not FlagModificandoUnidades) then
  begin
     FlagModificandoUnidades := True;
...
procedure TDMPropuestasConfirm.QMDetalleUNIDADES_LOGISTICASChange(Sender: TField);
begin
...
procedure TDMPropuestasConfirm.QMCabeceraCalcFields(DataSet: TDataSet);
begin
  QMCabeceraTITULO_ESTADO.AsString := DameTituloEstado(QMCabeceraESTADO.AsInteger);
end;

procedure TDMPropuestasConfirm.QMCabeceraVALOR_CAMB_FIJOChange(Sender: TField);
begin
...
procedure TDMPropuestasConfirm.QMCabeceraVALOR_CAMB_FIJO_INVChange(Sender: TField);
begin
  if (not ModificandoCambioFijo) then
  begin
     ModificandoCambioFijo := True;
...
procedure TDMPropuestasConfirm.Inserta(Articulo: string; Unidades: double; Lote: string; IdUbicacion: integer; Precio: double; Descripcion: string; PCoste, Descuento1, Descuento2, Descuento3: double; FechaPrevista: TDateTime);
...
procedure TDMPropuestasConfirm.AbrirCerrar(Estado: integer);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'UPDATE VER_CABECERAS_OCP SET ESTADO = ?ESTADO WHERE ID_E = ?ID_E';
        if (Estado = 0) then
...
procedure TDMPropuestasConfirm.CambiaEstadoDocumento(Estado: integer);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMPropuestasConfirm.AbreDocumento;
begin
  CambiaEstadoDocumento(0);
end;

procedure TDMPropuestasConfirm.CierraDocumento;
begin
  CambiaEstadoDocumento(5);
end;

procedure TDMPropuestasConfirm.QMDetalleUNIDADESGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetalleUNIDADES.AsFloat]);
end;

procedure TDMPropuestasConfirm.FiltraDocumento(Serie, Filtro: string);
begin
...
procedure TDMPropuestasConfirm.QMCabeceraBeforeClose(DataSet: TDataSet);
begin
  xProveedor.Close;
  xDirecciones.Close;
  QMDetalle.Close;
...
procedure TDMPropuestasConfirm.QMCabeceraDIRECCIONChange(Sender: TField);
begin
...
procedure TDMPropuestasConfirm.QMDetallePESOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetallePESO.AsFloat]);
end;

procedure TDMPropuestasConfirm.QMDetallePESO_TOTALGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetallePESO_TOTAL.AsFloat]);
end;

procedure TDMPropuestasConfirm.QMDetallePESO_REALGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetallePESO_REAL.AsFloat]);
end;

procedure TDMPropuestasConfirm.QMDetallePESO_REALChange(Sender: TField);
begin
  FlagModificandoUnidades := True;
  try
     if Param_VENULKG001 and (QMDetalleTIPO_UNIDAD_LOGISTICA.AsString = 'KG') then
...
procedure TDMPropuestasConfirm.QMDetalleUNIDADES_EXTGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetalleUNIDADES_EXT.AsFloat]);
end;

procedure TDMPropuestasConfirm.QMDetalleUNIDADES_SECGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetalleUNIDADES_SEC.AsFloat]);
end;

procedure TDMPropuestasConfirm.xProveedorAfterOpen(DataSet: TDataSet);
begin
  xAvisos.Open;
end;

procedure TDMPropuestasConfirm.xProveedorBeforeClose(DataSet: TDataSet);
begin
  xAvisos.Close;
end;

procedure TDMPropuestasConfirm.QMDetalleBeforeInsert(DataSet: TDataSet);
begin
  DMMain.RestingeEdicion(DataSet, QMCabeceraESTADO.AsInteger);
end;

procedure TDMPropuestasConfirm.QMCabeceraBeforeDelete(DataSet: TDataSet);
begin
  DMMain.RestingeEdicion(DataSet, QMCabeceraESTADO.AsInteger);
end;

procedure TDMPropuestasConfirm.QMDetalleBeforeDelete(DataSet: TDataSet);
begin
  DMMain.RestingeEdicion(DataSet, QMCabeceraESTADO.AsInteger);
end;

procedure TDMPropuestasConfirm.QMCabeceraTITULOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
