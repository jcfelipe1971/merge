...
type
  TFFiltra_Clientes_Agr = class(TFPEditSinNavegador)
...
var
  FFiltra_Clientes_Agr : TFFiltra_Clientes_Agr;

implementation

uses UDMMain, UDMClientes, UUtiles, UDameDato;

{$R *.dfm}

procedure TFFiltra_Clientes_Agr.CHKAndClick(Sender: TObject);
begin
...
procedure TFFiltra_Clientes_Agr.CHKorClick(Sender: TObject);
begin
...
procedure TFFiltra_Clientes_Agr.CHKNOTClick(Sender: TObject);
begin
...
procedure TFFiltra_Clientes_Agr.EFAgrupacionChange(Sender: TObject);
begin
  ETitulo.Text := DameTituloAgrupacion('C', EFAgrupacion.Text);
end;

procedure TFFiltra_Clientes_Agr.SBAnadeClick(Sender: TObject);
var
  Condicion : string;
begin
  if (CHKAnd.Checked = True) then
...
procedure TFFiltra_Clientes_Agr.SBBorraCondClick(Sender: TObject);
begin
...
procedure TFFiltra_Clientes_Agr.FormShow(Sender: TObject);
begin
...
procedure TFFiltra_Clientes_Agr.FormCreate(Sender: TObject);
begin
...
procedure TFFiltra_Clientes_Agr.SBAceptarClick(Sender: TObject);
var
  i : integer;
  Condicion, Agrupacion : string;
begin
  FMClientes.DataModule.Borra_TMP_agrupaciones;
  i := 1;
  ...
  begin
     ...
     begin
        Condicion := '';
        FMClientes.DataModule.Carga_agrupaciones(Agrupacion);
     end;

     if (Condicion = 'AND') then
        FMClientes.DataModule.Filtra_agrupaciones(Agrupacion, 0);

     if (Condicion = 'OR') then
     begin
        FMClientes.DataModule.Traspasa_agrupaciones;
        FMClientes.DataModule.Carga_agrupaciones(Agrupacion);
     end;

     if (Condicion = 'AND NOT') then
        FMClientes.DataModule.Filtra_agrupaciones(Agrupacion, 1);
