unit UDMAsistenteImpAcreedores;
...
type
  TDMAsistenteImpAcreedores = class(TDataModule)
     TLocal: THYTransaction;
     TUpdate: THYTransaction;
     QImportaAcreedor: THYFIBQuery;
...
var
  DMAsistenteImpAcreedores : TDMAsistenteImpAcreedores;

implementation

uses UDMMain, UEntorno, Variants, UUtiles, Dialogs, StrUtils, UHojaCalc, UFMAsistenteImpAcreedores, FIBDataSetRW;

{$R *.dfm}

procedure TDMAsistenteImpAcreedores.DataModuleCreate(Sender: TObject);
begin
...
procedure TDMAsistenteImpAcreedores.DataModuleDestroy(Sender: TObject);
begin
  Filas.Free;
end;

function TDMAsistenteImpAcreedores.LeerDatos(Archivo, Hoja: string): boolean;
var
  HCalc : THojaCalc;
  f, c, x, y : integer;
  ...
  try
     ...
     try
        ...
        try
           ...
           FMAsistenteImpAcreedores.ProgresoImportacion.Min := 0;
           FMAsistenteImpAcreedores.ProgresoImportacion.Max := y;
           ...
           begin
              ...
              FMAsistenteImpAcreedores.ProgresoImportacion.Position := c;
           end;
...
function TDMAsistenteImpAcreedores.DameFilas: TObjectList;
begin
  Result := Filas;
end;

function TDMAsistenteImpAcreedores.DameColumnas: TStrings;
begin
  Result := (Filas.Items[0] as TStrings);
end;

procedure TDMAsistenteImpAcreedores.RellenaCampos(Campos: TStrings);
var
...
begin
  ...
  try
     ...
     begin
        ...
        SelectSQL.Text := 'SELECT * FROM VER_ACREEDORES_IMPORTACION WHERE EMPRESA = 0';
        Open;
...
procedure TDMAsistenteImpAcreedores.InicializaImportacion(aSeparadorDecimal: char);
var
...
begin
  ...
  try
     DMAsistenteImpAcreedores.RellenaCampos(ListaCampos);
  ...
  with QImportaAcreedor do
  begin
     SQL.Clear;
     SQL.Add(' INSERT INTO VER_ACREEDORES_IMPORTACION');
     SQL.Add(' (' + Campos + ')');
     SQL.Add(' VALUES ');
     SQL.Add(' (' + Valores + ')');
  end;

  DMMain.Log(QImportaAcreedor.SQL.Text);
end;

procedure TDMAsistenteImpAcreedores.NuevoAcreedor;
var
  i : integer;
begin
  DMMain.Log('NuevoAcreedor');
  with QImportaAcreedor do
  begin
...
procedure TDMAsistenteImpAcreedores.AsignaParametro(Parametro, Valor: string);
var
  i, j : integer;
  s : string;
begin
     ...
     for i := 0 to Params.Count - 1 do
        ...
        begin
           ...
           begin
              ...
              begin
                 FMAsistenteImpAcreedores.Log(Format(_('Se reduce el tamaño del campo %s. (Max. = %d). Se importa: %s'), [Params[i].Name, Params[i].Size, Copy(Valor, 1, Params[i].Size)]));
                 FMAsistenteImpAcreedores.Log('');
              end;
...
procedure TDMAsistenteImpAcreedores.CrearAcreedor;
var
  i : integer;
begin
  // Crea el acreedor, rellenando algunos parametros si fuera necesario.
  DMMain.Log('CrearAcreedor');
  with QImportaAcreedor do
  begin
     for i := 0 to Params.Count - 1 do
     begin
...
procedure TDMAsistenteImpAcreedores.ActualizaDatosImportacion(aFileName: string; var aLastRow, aLastCol, aCountSheet: integer; var aActiveSheetName: string; Sheets: TStrings);
var
  i : integer;
begin
  aLastRow := 0;
