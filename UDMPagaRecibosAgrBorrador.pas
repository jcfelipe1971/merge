unit UDMPagaRecibosAgrBorrador;
...
type
  TDMPagaRecibosAgrBorrador = class(TDataModule)
  ...
  public
     ...
     function PagaRecibos(Signo, TipoFac, CtaPago: string; cod_cli_pro: integer; Fecha: TDateTime; Cantidad: double; CuentaUnificada: boolean): boolean;
     procedure Filtra(tipo: string; cod_cli_pro: integer);
     procedure CambiaCod_Cli(tipo: string; Cod_cli_pro: integer; var titulo: string);
     procedure AgrupaRecibos(TipoFac: string; Cod_cli_pro: integer; FormaPago: string; Fecha: TDateTime; Modo: smallint; TipoAsientoKri, Serie: string);
     ...
     procedure CalculaImporte(Cli_Pro: integer; TipoDoc: string; Fecha: TDateTime; var Importe: double; var Moneda: string);
...
var
  DMPagaRecibosAgrBorrador : TDMPagaRecibosAgrBorrador;

implementation

uses UFBusca, UEntorno, UDMLstFactura, UFormGest, UDMMain,
  UFMPagaRecibosAgrBorrador, UFPregAnticipo, UDMBorradorContabilidad, UFParada,
  UUtiles, UFMain;

{$R *.DFM}

procedure TDMPagaRecibosAgrBorrador.DMPagaRecibosAgrupCreate(Sender: TObject);
begin
...
procedure TDMPagaRecibosAgrBorrador.Filtra(tipo: string; cod_cli_pro: integer);
var
...
function TDMPagaRecibosAgrBorrador.PagaRecibos(Signo, TipoFac, CtaPago: string; cod_cli_pro: integer; Fecha: TDateTime; Cantidad: double; CuentaUnificada: boolean): boolean;
begin
  Result := False;
  ...
  // Hemos de postear la cabecera de borrador, para poder postear
  // su subdetalle

  // Primero se hace post de la cabecera de QMAsientos
  // Y de la  sub tabla pero solo en caso de que no este ya insertado.

  with DMBorradorContabilidad do
  begin
     if not (QMAsientosCartera.Active) then
        QMAsientosCartera.Open;
     QMAsientosCartera.Edit;
     QMAsientosCarteraMAESTRO.AsInteger := cod_cli_pro;
     QMAsientosCarteraSIGNO.AsString := Signo;
     QMAsientosCarteraCUENTA_CP.AsString := CtaPago;
     QMAsientosCarteraTIPO_FAC.AsString := TipoFac;
     if ((QMAsientos.State = dsInsert) or
        (QMAsientos.State = dsEdit)) then
        QMAsientos.Post;

     QMAsientosCarteraRIB.AsInteger := QMAsientosRIB.AsInteger;

     QMAsientosCartera.Post;
  end;
  ...
  try
     ...
     begin
        ...
        Params.ByName['ENTRADA'].AsInteger := REntorno.Entrada;
        Params.ByName['CUENTAUNIFICADA'].AsInteger := BoolToInt(CuentaUnificada);
        Params.ByName['TITULO'].AsString := DMBorradorContabilidad.QMAsientosTITULO.AsString;
        Params.ByName['RIB'].AsInteger := DMBorradorContabilidad.QMAsientosRIB.AsInteger;
        TFParada.Create(Self).ExecQuery(SPPaga);
        // dji lrk kri - si no muestra los errores pongo ExecQuery y quito el TFParada
        FreeHandle;
        Result := True;
     end;
  except
     ShowMessage(LimpiaMensaje(Exception(ExceptObject)));
     {
     dji lrk kri - si no muestra los errores pongo esto y quito lo otro
     on E: Exception do
  ...
  if (Result) then
  begin
     DMBorradorContabilidad.QMAsientosCartera.Close;
     QMRecibos.Close;
     QMRecibos.Open;
     DMBorradorContabilidad.QMAsientos.Refresh;
  end
  else
  begin
     // Borrar el registro introducido
     DMBorradorContabilidad.QMAsientosCartera.Delete;
     DMBorradorContabilidad.QMAsientosCartera.Close;
...
procedure TDMPagaRecibosAgrBorrador.BusquedaCompleja(Tipo: string; Cod_cli_pro: integer);
var
...
begin
  ...
  begin
     ...
     QMRecibos.Params.ByName['TIPO'].AsString := Tipo;
     QMRecibos.Params.ByName['TIPOAGRUPADO'].AsString := TipoAgrupado;
     QMRecibos.Params.ByName['TIPONUEVO'].AsString := TipoNuevo;
     QMRecibos.Params.ByName['COD_CLI_PRO'].AsInteger := cod_cli_pro;
  end;
  QMRecibos.Open;
end;

procedure TDMPagaRecibosAgrBorrador.CambiaCod_Cli(Tipo: string; Cod_cli_pro: integer; var Titulo: string);
begin
  Importe := 0;
  if Tipo = 'FAC' then
     with xClientes do
  ...
  FMPagaRecibosAgrBorrador.Importe(Importe, REntorno.Moneda);
end;

procedure TDMPagaRecibosAgrBorrador.QMRecibosAfterPost(DataSet: TDataSet);
begin
  TFIBTableSet(DataSet).Transaction.CommitRetaining;
end;

procedure TDMPagaRecibosAgrBorrador.QMRecibosENTRADAChange(Sender: TField);
var
...
begin
  if (Sender.AsInteger = REntorno.Entrada) then
     ...
     Recibos := -1;
  FMPagaRecibosAgrBorrador.ObtDatosCli_Pro(Cli_Pro, Procedimiento,
     Signo, Tipo, Cuenta, Cantidad);
  ...
  begin
     ...
     begin
        ...
        Importe := Importe + (QMRecibosLIQUIDO.AsFloat * Recibos);
        FMPagaRecibosAgrBorrador.Importe(Importe, QMRecibosMONEDA.AsString);
     end;
  end
  else
  begin
  ...
  FMPagaRecibosAgrBorrador.PMain.Enabled := True;
end;

function TDMPagaRecibosAgrBorrador.DameFormaPago(Tipo: string): string;
begin
  if Tipo = 'FAC' then
...
procedure TDMPagaRecibosAgrBorrador.AgrupaRecibos(TipoFac: string; Cod_cli_pro: integer; FormaPago: string; Fecha: TDateTime; Modo: smallint; TipoAsientoKri, Serie: string);
var
  Registro : integer;
  Signo : char;
begin
...
function TDMPagaRecibosAgrBorrador.EsAgrupado: boolean;
begin
...
function TDMPagaRecibosAgrBorrador.Dameregistro: integer;
begin
  Result := QMRecibosREGISTRO.AsInteger;
end;

procedure TDMPagaRecibosAgrBorrador.LimpiaEntradas;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMPagaRecibosAgrBorrador.Anticipos(fecha: TDateTime);
var
  Saldo, Tope : double;
  Tipo, Moneda : string;
begin
  // Saldo(Fecha, S, Moneda);
  ...
  begin
     ...
     if (Tope < Saldo) then
        Saldo := Tope;
     if TFPregAnticipo.Create(Self).Mostrar(Saldo, Moneda) then
     begin
        with THYFIBQuery.Create(nil) do
        begin
           try
...
procedure TDMPagaRecibosAgrBorrador.QMRecibosBeforePost(DataSet: TDataSet);
begin
...
procedure TDMPagaRecibosAgrBorrador.QMRecibosliquido_monedaGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := FormatFloat(DMMain.MascaraMoneda(QMRecibosMONEDA.AsString, 1),
     QMRecibosLIQUIDO.AsFloat);
end;

procedure TDMPagaRecibosAgrBorrador.PonImporte(valor: double);
begin
  Importe := valor;
end;

procedure TDMPagaRecibosAgrBorrador.QMRecibosNewRecord(DataSet: TDataSet);
begin
  DataSet.Cancel;
end;

procedure TDMPagaRecibosAgrBorrador.QMRecibosBeforeInsert(DataSet: TDataSet);
begin
  DataSet.Cancel;
end;

procedure TDMPagaRecibosAgrBorrador.InsCPRecMarc(Cli_Pro: integer; Signo, Tipo, Cuenta: string; Cantidad, Importe: double; entrada, recibos: integer);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Add(' EXECUTE PROCEDURE E_CARTERA_INS_REC_MARC_BORR ');
        SQL.Add(' (?COD_CLI_PRO, ?SIGNO, ?TIPO, ?CUENTA, ?CANTIDAD, ?IMPORTE, ?ENTRADA, ?RECIBOS) ');
...
procedure TDMPagaRecibosAgrBorrador.LimpiaCPRecMarc;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'DELETE FROM TMP_CARTERA_CP_REC_MARC_BORR WHERE ENTRADA = :ENTRADA';
        Params.ByName['ENTRADA'].AsInteger := REntorno.Entrada;
        ExecQuery;
        FreeHandle;
     finally
...
procedure TDMPagaRecibosAgrBorrador.ActCPRecMarc(Cli_Pro: integer; Signo, Tipo_Doc, Cuenta: string; Fecha: TDateTime; Cantidad: double);
begin
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Add(' UPDATE TMP_CARTERA_CP_REC_MARC_BORR ');
           SQL.Add(' SET ');
...
procedure TDMPagaRecibosAgrBorrador.CalculaImporte(Cli_Pro: integer; TipoDoc: string; Fecha: TDateTime; var Importe: double; var Moneda: string);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Add(' EXECUTE PROCEDURE E_CARTERA_CAL_IMP_REC_MARC_BORR ');
        SQL.Add(' (?EMPRESA, ?EJERCICIO_AGRUPACION, ?CANAL, ?TIPO_DOC, ?COD_CLI_PRO, ?ENTRADA, ?FECHA) ');
        Params.ByName['EMPRESA'].AsInteger := REntorno.Empresa;
        Params.ByName['EJERCICIO_AGRUPACION'].AsInteger := REntorno.Ejercicio;
        Params.ByName['CANAL'].AsInteger := REntorno.Canal;
...
procedure TDMPagaRecibosAgrBorrador.QMCPRecMarcadosAfterOpen(DataSet: TDataSet);
begin
  QMCPRecMarcadosIMPORTE.DisplayFormat := MascaraN;
end;

function TDMPagaRecibosAgrBorrador.DameClienteCartera(IdCarteraDetalle: integer): integer;
begin
...
procedure TDMPagaRecibosAgrBorrador.SeleccionaRecibo(IdCarteraDetalle: integer);
begin
...
function TDMPagaRecibosAgrBorrador.DameAcreedorCuenta(Cuenta: string): integer;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMPagaRecibosAgrBorrador.QMRecibosCalcFields(DataSet: TDataSet);
begin
