...
type
  TDMAsistenteImpClientes = class(TDataModule)
     TLocal: THYTransaction;
     TUpdate: THYTransaction;
     QImportarCliente: THYFIBQuery;
...
var
  DMAsistenteImpClientes : TDMAsistenteImpClientes;

implementation

uses UDMMain, UEntorno, Variants, UUtiles, Dialogs, StrUtils, UHojaCalc, UFMAsistenteImpClientes, FIBDataSetRW;

{$R *.dfm}

procedure TDMAsistenteImpClientes.DataModuleCreate(Sender: TObject);
begin
...
procedure TDMAsistenteImpClientes.DataModuleDestroy(Sender: TObject);
begin
  Filas.Free;
end;

function TDMAsistenteImpClientes.LeerDatos(Archivo, Hoja: string): boolean;
var
  HCalc : THojaCalc;
  f, c, x, y : integer;
  ...
  try
     ...
     try
        ...
        try
           ...
           FMAsistenteImpClientes.ProgresoImportacion.Min := 0;
           FMAsistenteImpClientes.ProgresoImportacion.Max := y;
...
function TDMAsistenteImpClientes.DameFilas: TObjectList;
begin
  Result := Filas;
end;

function TDMAsistenteImpClientes.DameColumnas: TStrings;
begin
  Result := (Filas.Items[0] as TStrings);
end;

procedure TDMAsistenteImpClientes.RellenaCampos(Campos: TStrings);
var
...
begin
  ...
  try
     ...
     begin
        ...
        SelectSQL.Text := 'SELECT * FROM VER_CLIENTES_IMPORTACION WHERE EMPRESA = 0';
        Open;
...
procedure TDMAsistenteImpClientes.InicializaImportacion(aSeparadorDecimal: char);
var
...
begin
  ...
  try
     DMAsistenteImpClientes.RellenaCampos(ListaCampos);
  ...
  with QImportarCliente do
  begin
     SQL.Clear;
     SQL.Add(' INSERT INTO VER_CLIENTES_IMPORTACION');
     SQL.Add(' (' + Campos + ')');
     SQL.Add(' VALUES ');
     SQL.Add(' (' + Valores + ')');
  end;

  DMMain.Log(QImportarCliente.SQL.Text);
end;

procedure TDMAsistenteImpClientes.NuevoCliente;
var
  i : integer;
begin
  DMMain.Log('NuevoCliente');
  with QImportarCliente do
  begin
...
procedure TDMAsistenteImpClientes.AsignaParametro(Parametro, Valor: string);
var
  i, j : integer;
  s : string;
begin
     ...
     for i := 0 to Params.Count - 1 do
        ...
        begin
           ...
           begin
              ...
              begin
                 FMAsistenteImpClientes.Log(Format(_('Se reduce el tamaño del campo %s. (Max. = %d). Se importa: %s'), [Params[i].Name, Params[i].Size, Copy(Valor, 1, Params[i].Size)]));
                 FMAsistenteImpClientes.Log('');
              end;
...
procedure TDMAsistenteImpClientes.CrearCliente;
var
  i : integer;
begin
  // Crea el cliente, rellenando algunos parametros si fuera necesario.
  DMMain.Log('CrearCliente');
  with QImportarCliente do
  begin
     for i := 0 to Params.Count - 1 do
     begin
...
procedure TDMAsistenteImpClientes.ActualizaDatosImportacion(aFileName: string; var aLastRow, aLastCol, aCountSheet: integer; var aActiveSheetName: string; Sheets: TStrings);
var
  i : integer;
begin
  aLastRow := 0;
