...
uses {IDIOMA_CODE}gnugettext{IDIOMA_CODE},
  ...
  FIBTableDataSetRO, FIBDataSet, ImgList, ActnList, UControlEdit,
  FIBDataSetRO, FIBQuery, HYFIBQuery, Mask, rxToolEdit, ULFActionList,
  ULFToolBar, ULFPanel, ULFComboBox, ULFLabel, ULFDateEdit, ULFEdit;
...
type
  ...
  private
     ...
     procedure GuardaFiltro(Tabla: TFibTableSetRO; Filtro: string; Abrir: boolean; Serie: string; Posicion: smallint);
     procedure RecuperaFiltro(Tabla: TFibTableSetRO);
  ...
  public
     ...
     FSQL: string;
     Filtros: string;
     function SeleccionaBusqueda(Tabla: TFibTableSetRO; Filtro: string = '00000'; Abrir: boolean = True; Serie: string = ''; Posicion: smallint = 3): integer;
  end;
...
begin
  ...
  FPField := TFBuscaRO(Parent).FFields[cbFields.ItemIndex];
end;

procedure TPanelSearch.OnFieldsKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
  if (Key = VK_RETURN) then
     TFBuscaRO(Parent).act_ConfirmarExecute(Sender);
  if (Key = VK_ESCAPE) then
     TFBuscaRO(Parent).act_CancelarExecute(Sender);
end;
...
procedure TFBuscaRO.FormDestroy(Sender: TObject);
var
  ix : integer;
  PRec : PFieldRec;
  PDate : PFieldDate;
...
procedure TFBuscaRO.SetDateComboBox;
var
  index : integer;
  PDate : PFieldDate;
begin
...
procedure TFBuscaRO.AddNewPanelToList(Kind: integer);
var
...
procedure TFBuscaRO.RemoveLastPanel;
var
  Panel : TPanelSearch;
  iHeight : integer;
begin
...
procedure TFBuscaRO.BuildConditionalSQL(Panel: TPanelSearch);
begin
...
procedure TFBuscaRO.CambiaCampo(Sender: TObject);
begin
...
procedure TFBuscaRO.BuildNucliSQL;
var
  ix : integer;
begin
  {Nucleo simple}
...
procedure TFBuscaRO.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  REntorno.ModoEnter := modoReturn;
  Action := caFree;
end;

procedure TFBuscaRO.cbDateClick(Sender: TObject);
begin
  {Guardamos el nuevo indice }
  FPDate := FDates[cbDate.ItemIndex];
end;

procedure TFBuscaRO.tbNoDateClick(Sender: TObject);
begin
  if FDateCount = 0 then
     Exit;
...
function TFBuscaRO.SeleccionaBusqueda(Tabla: TFibTableSetRO; Filtro: string = '00000'; Abrir: boolean = True; Serie: string = ''; Posicion: smallint = 3): integer;
var
...
procedure TFBuscaRO.act_MasExecute(Sender: TObject);
var
  Paneles : smallint;
begin
  {Añadimos un nuevo panel a la lista}
...
procedure TFBuscaRO.act_ConfirmarExecute(Sender: TObject);
var
  ix : integer;
begin
  FSQL := 'select * ';
...
procedure TFBuscaRO.act_CancelarExecute(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TFBuscaRO.act_MenosExecute(Sender: TObject);
begin
...
procedure TFBuscaRO.act_NoFiltroExecute(Sender: TObject);
var
  andStr : string;
begin
  FSQL := 'select * ';
...
procedure TFBuscaRO.AddFields(Descripcion, Field, Table, Key, KeyMaster: string; Tipo: integer; Filtra: boolean);
var
  PRecord : PFieldRec;
begin
  if not Filtra then
...
procedure TFBuscaRO.AddDate(Descripcion, Field, Table, Key, KeyMaster: string; Filtra: boolean);
var
  PRecord : PFieldDate;
begin
  if not Filtra then
...
procedure TFBuscaRO.FormShow(Sender: TObject);
var
...
begin
  ...
  begin
     ...
     PanelAct.FPField := FFields[RFiltro.cbFields];
...
procedure TFBuscaRO.GuardaFiltro(Tabla: TFibTableSetRO; Filtro: string; Abrir: boolean; Serie: string; Posicion: smallint);
var
...
procedure TFBuscaRO.RecuperaFiltro(Tabla: TFibTableSetRO);
var
...
procedure TFBuscaRO.FormCreate(Sender: TObject);
begin
...
function TFBuscaRO.QuitaComaPonPunto(Parametro: string): string;
var
  Posicion : integer;
  STemp : string;
begin
...
function TFBuscaRO.ArreglaOrderBY(orden: string): string;
var
  aux : string;
begin
...
procedure TFBuscaRO.act_ConfigurarExecute(Sender: TObject);
begin
