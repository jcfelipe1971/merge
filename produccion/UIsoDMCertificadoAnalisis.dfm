object IsoDMCertificadoAnalisis: TIsoDMCertificadoAnalisis
  ...
  object QMFichaC: TFIBTableSet
    ...
    CachedUpdates = False
    DeleteSQL.Strings = (
      'DELETE FROM ISO_ENSAYO_ORD_CAB'
      'WHERE'
      '  ID_ENSAYO=?old_ID_ENSAYO AND '
      '  EMPRESA=?old_EMPRESA ')
    InsertSQL.Strings = (
      'INSERT INTO ISO_ENSAYO_ORD_CAB'
      
        '  (ID_ENSAYO,ARTICULO,FECHA_ENSAYO,REVISION,RESPONSABLE,ACTIVO,N' +
        'OTAS,UNIDAD_ENVASE,UNIDAD_TRANSPORTE,LOTE_HOMOGENEO,EMPRESA,ID_L' +
        'OTE,CLIENTE,ID_ORDEN,FICHA,REFERENCIA,DESCRIPCION,COLORES,NOMBRE' +
        '_PRODUCTO,APLICACION_PREFERENTE, CAR_NORMAS_UNE, IMPRIME_VALOR_2' +
        ')'
      'VALUES'
      
        '  (?ID_ENSAYO,?ARTICULO,?FECHA_ENSAYO,?REVISION,?RESPONSABLE,?AC' +
        'TIVO,?NOTAS,?UNIDAD_ENVASE,?UNIDAD_TRANSPORTE,?LOTE_HOMOGENEO,?E' +
        'MPRESA,?ID_LOTE,?CLIENTE,?ID_ORDEN,?FICHA,?REFERENCIA,?DESCRIPCI' +
        'ON,?COLORES,?NOMBRE_PRODUCTO,?APLICACION_PREFERENTE, ?CAR_NORMAS' +
        '_UNE,?IMPRIME_VALOR_2)')
    RefreshSQL.Strings = (
      'SELECT'
      '  *'
      'FROM ISO_ENSAYO_ORD_CAB'
      'WHERE'
      '  ID_ENSAYO=?ID_ENSAYO AND '
      '  EMPRESA=?EMPRESA ')
    SelectSQL.Strings = (
      'SELECT * FROM ISO_ENSAYO_ORD_CAB'
      'WHERE EMPRESA=?EMPRESA'
      'ORDER BY ID_ENSAYO')
    UniDirectional = False
    UpdateRecordTypes = [cusUnmodified, cusModified, cusInserted]
    UpdateSQL.Strings = (
      'UPDATE ISO_ENSAYO_ORD_CAB'
      'SET'
      '  ARTICULO=?ARTICULO '
      ...
      '  ,IMPRIME_VALOR_2=?IMPRIME_VALOR_2'
      'WHERE'
      '  ID_ENSAYO=?ID_ENSAYO AND '
      '  EMPRESA=?EMPRESA ')
    ...
    object QMFichaCIMPRIME_VALOR_2: TIntegerField
      DisplayLabel = 'Imprime Valor 2'
      FieldName = 'IMPRIME_VALOR_2'
    end
  end
  ...
  object QMFichaD: TFIBTableSet
    ...
    CachedUpdates = False
    DeleteSQL.Strings = (
      'DELETE FROM ISO_ENSAYO_ORD_DET'
      'WHERE'
      '  EMPRESA=?old_EMPRESA AND '
      '  ID_ENSAYO=?old_ID_ENSAYO AND '
      '  LINEA=?old_LINEA ')
    InsertSQL.Strings = (
      'INSERT INTO ISO_ENSAYO_ORD_DET'
    ...
    RefreshSQL.Strings = (
      'SELECT'
      '  *'
      'FROM ISO_ENSAYO_ORD_DET'
      'WHERE'
      '  EMPRESA=?EMPRESA AND '
      '  ID_ENSAYO=?ID_ENSAYO AND '
      '  LINEA=?LINEA ')
    SelectSQL.Strings = (
      'SELECT * FROM ISO_ENSAYO_ORD_DET'
      'WHERE EMPRESA=?EMPRESA AND ID_ENSAYO=?ID_ENSAYO'
      'ORDER BY LINEA')
    UniDirectional = False
    UpdateRecordTypes = [cusUnmodified, cusModified, cusInserted]
    UpdateSQL.Strings = (
      'UPDATE ISO_ENSAYO_ORD_DET'
      'SET'
      '  DESCRIPCION=?DESCRIPCION '
  ...
  object frIsoCertificadoAnalisis: TfrHYReport
  ...
  object SPLineaSegDet: THYFIBQuery
    ...
    ParamCheck = True
    SQL.Strings = (
      'select max(linea) linea from iso_ensayo_ORD_det'
      'where (empresa=?empresa and id_ensayo=?id_ensayo)')
    Transaction = TLocal
    AutoTrans = True
    Left = 256
  ...
  object SPDuplicaFicha: THYFIBQuery
    Database = DMMain.DataBase
    ParamCheck = True
    SQL.Strings = (
      
        'execute procedure iso_duplica_detalle_ensayo (?id_orden,?cliente' +
        ',?id_lote,?compuesto,?empresa)')
    Transaction = TLocal
    AutoTrans = True
    Left = 256
    Top = 176
