unit UFMAsistenteImpProveedores;
...
type
  TFMAsistenteImpProveedores = class(TFPEditSinNavegador)
  ...
  private
     ...
     procedure ImportarProveedores(BorraLog: boolean = True);
  public
     { Public declarations }
...
var
  FMAsistenteImpProveedores : TFMAsistenteImpProveedores;

implementation

uses UDMMain, UEntorno, UDMAsistenteImpProveedores, Contnrs, UUtiles;

{$R *.dfm}

procedure TFMAsistenteImpProveedores.FormCreate(Sender: TObject);
var
  i : integer;
  Campos : TStrings;
  MI : TMenuItem;
begin
  ...
  AbreData(TDMAsistenteImpProveedores, DMAsistenteImpProveedores);
  InicializaVariables;

  EdArchivoImp.Text := LeeDatoIni('ImportacionProveedores', 'UltimoArchivo', '');
  CBHojaAImportar.Text := LeeDatoIni('ImportacionProveedores', 'UltimaHoja', '');
  ...
  try
     DMAsistenteImpProveedores.RellenaCampos(Campos);
...
procedure TFMAsistenteImpProveedores.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  CierraData(DMAsistenteImpProveedores);
  Action := caFree;
end;

{ PROCEDIMIENTOS DEL MOTOR DEL ASISTENTE }

procedure TFMAsistenteImpProveedores.InicializaVariables;
begin
  // Expandir nodo
...
procedure TFMAsistenteImpProveedores.ActivarTS(Index: integer);
var
  i : integer;
begin
...
procedure TFMAsistenteImpProveedores.ButtAntClick(Sender: TObject);
begin
  MueveTab(-1);
end;

procedure TFMAsistenteImpProveedores.ButtNextClick(Sender: TObject);
begin
  MueveTab(1);
end;

procedure TFMAsistenteImpProveedores.MueveTab(Valor: integer);
begin
  Cambiar := False;
...
procedure TFMAsistenteImpProveedores.ActivarTree(Index: integer);
begin
  TVMain.Items[0].Item[index].Selected := True;
end;

procedure TFMAsistenteImpProveedores.TVMainCollapsed(Sender: TObject; Node: TTreeNode);
begin
  TVMain.Items[0].Expanded := True;
end;

procedure TFMAsistenteImpProveedores.TVMainChange(Sender: TObject; Node: TTreeNode);
begin
  MueveTabTree(Node.Index);
end;

procedure TFMAsistenteImpProveedores.ControlaBotones(Index: integer);
begin
  if (TabIndice = 0) then
...
procedure TFMAsistenteImpProveedores.MueveTabTree(Index: integer);
begin
  if not (Cambiar) then
  begin
...
function TFMAsistenteImpProveedores.CheckTab(Index: integer): boolean;
begin
...
procedure TFMAsistenteImpProveedores.ButtCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TFMAsistenteImpProveedores.ButtSelArchivoClick(Sender: TObject);
var
...
begin
  ...
  begin
     ...
     try
        ...
        DMAsistenteImpProveedores.ActualizaDatosImportacion(Archivo, Filas, Columnas, CantidadHojas, HojaActiva, Sheets);
...
procedure TFMAsistenteImpProveedores.RefrescaInformacionArchivo;
var
  Filas, Columnas, CantidadHojas : integer;
  HojaActiva : string;
  Sheets : TStrings;
begin
  ...
  try
     HojaActiva := CBHojaAImportar.Text;
     DMAsistenteImpProveedores.ActualizaDatosImportacion(EdArchivoImp.Text, Filas, Columnas, CantidadHojas, HojaActiva, Sheets);
...
procedure TFMAsistenteImpProveedores.TSSeleccionArchivoShow(Sender: TObject);
begin
  EdArchivoImp.SetFocus;
end;

function TFMAsistenteImpProveedores.CheckArchivoImp: boolean;
begin
  Result := False;
  ...
  Result := DMAsistenteImpProveedores.LeerDatos(EdArchivoImp.Text, CBHojaAImportar.Text);
  if (Result) then
  begin
     LlenarGrid;
     LlenarConstantes;

     EscribeDatoIni('ImportacionProveedores', 'UltimoArchivo', Trim(EdArchivoImp.Text));
     EscribeDatoIni('ImportacionProveedores', 'UltimaHoja', Trim(CBHojaAImportar.Text));
     EscribeDatoIni(Trim(EdArchivoImp.Text) + '||' + Trim(CBHojaAImportar.Text), 'FechaImportacion', Now);

     LFCBPrimeraFilaTitulos.Checked := LeeDatoIni('ImportacionProveedores', 'PrimeraFilaTitulos', False);
     ESeparadorDecimal.Text := LeeDatoIni('ImportacionProveedores', 'SeparadorDecimal', '');
  end;
end;

procedure TFMAsistenteImpProveedores.LlenarGrid;
var
  ColCount, x, f, c : integer;
  Filas : TObjectList;
begin
  Filas := DMAsistenteImpProveedores.DameFilas;
...
procedure TFMAsistenteImpProveedores.LlenarConstantes;
var
  i : integer;
  Campos : TStrings;
begin
...
procedure TFMAsistenteImpProveedores.SGDatosImpDrawCell(Sender: TObject; ACol, ARow: integer; Rect: TRect; State: TGridDrawState);
begin
...
procedure TFMAsistenteImpProveedores.LimpiarLog;
begin
  MEMInfoProc.Lines.Clear;
end;

procedure TFMAsistenteImpProveedores.Log(Texto: string; NuevaLinea: boolean = True);
begin
  if NuevaLinea then
...
procedure TFMAsistenteImpProveedores.ImportarProveedores(BorraLog: boolean = True);
var
...
begin
  ...
  DMAsistenteImpProveedores.InicializaImportacion(ESeparadorDecimal.Text[1]);
  Seguir := True;
  f := PrimeraFila;
  ...
  begin
     DMAsistenteImpProveedores.NuevoProveedor;
     ...
     try
        ...
        begin
           ...
           begin
              DMAsistenteImpProveedores.AsignaParametro(SGDatosImp.Cells[c, 0], SGDatosImp.Cells[c, f]);
     ...
     try
        ...
        begin
           ...
           begin
              DMAsistenteImpProveedores.AsignaParametro(VLEValoresConstantes.Keys[c], VLEValoresConstantes.Values[VLEValoresConstantes.Keys[c]]);
     ...
     try
        DMAsistenteImpProveedores.CrearProveedor;
     except
        on e: Exception do
        begin
           Seguir := False;
           Log(_('Error al importar proveedor'));
           Log('----------');
           Log(E.Message);
           Log('----------');
           for c := 0 to SGDatosImp.ColCount - 1 do
...
procedure TFMAsistenteImpProveedores.ButtCrearClick(Sender: TObject);
begin
  ImportarProveedores;
end;

procedure TFMAsistenteImpProveedores.ButtCerrarClick(Sender: TObject);
begin
  Close;
end;

procedure TFMAsistenteImpProveedores.PMCamposDisponiblesClick(Sender: TObject);
begin
...
procedure TFMAsistenteImpProveedores.LFCBPrimeraFilaTitulosChange(Sender: TObject);
begin
  if LFCBPrimeraFilaTitulos.Checked then
  ...
  EscribeDatoIni('ImportacionProveedores', 'PrimeraFilaTitulos', LFCBPrimeraFilaTitulos.Checked);
end;

procedure TFMAsistenteImpProveedores.VLEValoresConstantesExit(Sender: TObject);
var
  i : integer;
begin
  // Primera linea contiene los titulos ("Key" - "Value")
...
procedure TFMAsistenteImpProveedores.EdArchivoImpChange(Sender: TObject);
begin
  inherited;
  RefrescaInformacionArchivo;
end;

procedure TFMAsistenteImpProveedores.CBHojaAImportarChange(Sender: TObject);
begin
  inherited;
  RefrescaInformacionArchivo;
end;

procedure TFMAsistenteImpProveedores.TSImportarShow(Sender: TObject);
begin
  inherited;
  EUltimaFilaImportada.Text := IntToStr(LeeDatoIni(Trim(EdArchivoImp.Text) + '||' + Trim(CBHojaAImportar.Text), 'UltimaFilaImportada', 0));
end;

procedure TFMAsistenteImpProveedores.BContinuarClick(Sender: TObject);
begin
  inherited;
...
procedure TFMAsistenteImpProveedores.ESeparadorDecimalChange(Sender: TObject);
begin
  inherited;
  EscribeDatoIni('ImportacionProveedores', 'SeparadorDecimal', ESeparadorDecimal.Text);
end;
