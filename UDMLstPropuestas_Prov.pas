...
type
  TDMLstPropuestas_prov = class(TDataModule)
     frPropuestas: TfrHYReport;
     ...
     procedure frPropuestasGetValue(const ParName: string; var ParValue: variant);
     procedure DMLstPropuestas_provCreate(Sender: TObject);
     procedure DMLstPropuestas_provDestroy(Sender: TObject);
     procedure frPropuestasEnterRect(Memo: TStringList; View: TfrView);
     ...
     procedure frPropuestasPrintReport;
     procedure HYReportPrintReport(Sender: THYReport);
     procedure QMDetalleAfterOpen(DataSet: TDataSet);
     procedure QMCabeceraAfterOpen(DataSet: TDataSet);
     procedure frxPropuestasBeforePrint(Sender: TfrxReportComponent);
     procedure frxPropuestasGetValue(const VarName: string; var Value: variant);
     ...
     procedure frPropuestasUserFunction(const Name: string; p1, p2, p3: variant; var Val: variant);
     function frxPropuestasUserFunction(const MethodName: string; var Params: variant): variant;
...
var
  DMLstPropuestas_prov : TDMLstPropuestas_prov;
  SW, filtroGen, id_s_aux : integer;
  serieGen : string;

implementation

uses UDMMain, UEntorno, UDMPropuestas, UDMListados, UFormGest,
  UUtiles, UDameDato, UParam;

{$R *.DFM}

procedure TDMLstPropuestas_prov.MostrarListadoFiltrado(Modo: byte; Filtro: integer; serie: string);
var
  Titulo : string;
...
begin
  ...
  Grupo := 518;
  Listado := 0;
  Titulo := _('PropuestaProveedor') + '_' + QMCabecera.FieldByName('EJERCICIO').AsString + '-' + QMCabecera.FieldByName('SERIE').AsString + '-' + QMCabecera.FieldByName('RIG').AsString;
  DMListados.Imprimir(Grupo, Listado, Modo, Serie, Titulo, frPropuestas, frxPropuestas, HYReport, HYReportMail);
end;

procedure TDMLstPropuestas_prov.MostrarListado(Modo: byte; serie: string);
var
  Titulo : string;
  Grupo, Listado : integer;
begin
  id_s_aux := DMPropuestas.QMCabecera.FieldByName('ID_E').AsInteger;
  SerieGen := Serie;
  SW := 1;
  with QMCabecera do
  begin
     Close;
     Params.ByName['ID_E'].AsInteger := DMPropuestas.QMCabecera.FieldByName('ID_E').AsInteger;
     Open;
  end;
...
procedure TDMLstPropuestas_prov.frPropuestasGetValue(const ParName: string; var ParValue: variant);
var
  i : integer;
  Cambio : extended;
begin
...
procedure TDMLstPropuestas_prov.DMLstPropuestas_provCreate(Sender: TObject);
begin
  ...
  frxPropuestas.AddFunction('function DAME_TEXTO_IDIOMA(i: integer; Bandera: string = ''''): string', 'Merge', 'Devuelve el texto en el idioma del documento');
end;

procedure TDMLstPropuestas_prov.DMLstPropuestas_provDestroy(Sender: TObject);
begin
  ric.Free;
...
procedure TDMLstPropuestas_prov.frPropuestasEnterRect(Memo: TStringList; View: TfrView);
begin
...
procedure TDMLstPropuestas_prov.QMDetalleAfterScroll(DataSet: TDataSet);
begin
  if ((QMDetalle.RecordCount > 0) and (ric <> nil)) then
     ric.Assign(QMDetalle.FieldByName('NOTAS'));
end;

procedure TDMLstPropuestas_prov.frUDNotasArtCheckEOF(Sender: TObject; var EOF: boolean);
begin
  EOF := (ric.Count = j);
end;

procedure TDMLstPropuestas_prov.frUDNotasArtFirst(Sender: TObject);
begin
  j := 0;
end;

procedure TDMLstPropuestas_prov.frUDNotasArtNext(Sender: TObject);
begin
  j := j + 1;
end;

procedure TDMLstPropuestas_prov.MostrarListadoMail(id_e: integer; serie: string; Modo: integer; const Ejercicio: integer; var Memo: TStrings);
var
  str : string;
begin
  id_s_aux := id_e;
  ...
  nombreFich := format('%s_%d-%s-%.5d.pdf', [_('Propuesta'), QMCabecera.FieldByName('EJERCICIO').AsInteger, QMCabecera.FieldByName('SERIE').AsString, QMCabecera.FieldByName('RIG').AsInteger]);
  ...
  DMListados.CargarMail(518, serie);
  ...
  begin
     ...
     begin
        with frPropuestas do
        begin
           LoadFromFIB(REntorno.Formato, str);
           PrepareReport;
           rutaFich := ExportarPDF(frPropuestas, nombreFich);
        end;
     end
     else
     if (REntorno.TipoListado = 'FR3') then
     begin
        with frxPropuestas do
        begin
           LoadFromFIB(REntorno.Formato, str);
           PrepareReport;
           rutaFich := ExportarPDF(frxPropuestas, nombreFich);
        end;
     end
     else
     begin
...
procedure TDMLstPropuestas_prov.HYReportMailEnterFields(Sender: THYReportMail; Field: THYField; var Source: string);
begin
  MatricialesEnterFields(Source);
end;

procedure TDMLstPropuestas_prov.MatricialesEnterFields(var Source: string);
var
  cambio : extended;
begin
...
procedure TDMLstPropuestas_prov.HYReportEnterFields(Sender: THYReport; Field: THYField; var Source: string);
begin
  MatricialesEnterFields(Source);
end;

procedure TDMLstPropuestas_prov.UpdateaDocumentos;
var
  ID_S : integer;
begin
...
procedure TDMLstPropuestas_prov.frPropuestasPrintReport;
begin
  UpdateaDocumentos;
end;

procedure TDMLstPropuestas_prov.HYReportPrintReport(Sender: THYReport);
begin
  UpdateaDocumentos;
end;

procedure TDMLstPropuestas_prov.QMDetalleAfterOpen(DataSet: TDataSet);
begin
  xArticulos.Open;
...
procedure TDMLstPropuestas_prov.QMCabeceraAfterOpen(DataSet: TDataSet);
begin
  xProveedores.Open;
...
procedure TDMLstPropuestas_prov.frxPropuestasBeforePrint(Sender: TfrxReportComponent);
begin
...
procedure TDMLstPropuestas_prov.frxPropuestasGetValue(const VarName: string; var Value: variant);
begin
  frPropuestasGetValue(VarName, Value);
end;

procedure TDMLstPropuestas_prov.QMCabeceraBeforeClose(DataSet: TDataSet);
begin
  xProveedores.Close;
...
procedure TDMLstPropuestas_prov.xProveedoresAfterOpen(DataSet: TDataSet);
begin
  xTerceros.Open;
end;

procedure TDMLstPropuestas_prov.xProveedoresBeforeClose(DataSet: TDataSet);
begin
  xTerceros.Close;
end;

procedure TDMLstPropuestas_prov.QMDetalleBeforeClose(DataSet: TDataSet);
begin
  xArticulos.Close;
...
procedure TDMLstPropuestas_prov.frPropuestasUserFunction(const Name: string; p1, p2, p3: variant; var Val: variant);
var
  Idioma : string;
  Grupo : integer;
  Parser : TfrParser;
...
function TDMLstPropuestas_prov.frxPropuestasUserFunction(const MethodName: string; var Params: variant): variant;
var
  Idioma : string;
  Grupo : integer;
  // Parser : TfrxParser;
...
procedure TDMLstPropuestas_prov.QMGrupoTallaAfterOpen(DataSet: TDataSet);
begin
  QMTituloTalla.Open;
  QMDetalleTallas.Open;
end;

procedure TDMLstPropuestas_prov.QMGrupoTallaBeforeClose(DataSet: TDataSet);
begin
  QMTituloTalla.Close;
  QMDetalleTallas.Close;
end;

procedure TDMLstPropuestas_prov.QMDetalleTallasAfterOpen(DataSet: TDataSet);
begin
  QMArticuloTallas.Open;
end;

procedure TDMLstPropuestas_prov.QMDetalleTallasBeforeClose(DataSet: TDataSet);
begin
  QMArticuloTallas.Close;
end;

procedure TDMLstPropuestas_prov.DobleCabeceraAfterScroll(DataSet: TDataSet);
begin
  xDobleIva.Close;
  xDobleIva.Open;
end;
