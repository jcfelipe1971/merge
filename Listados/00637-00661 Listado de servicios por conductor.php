<?php
	...
	foreach( $arr as $k => $conductor) {
		...
		$pdf->MultiCellSibiling( 150, $h, (($k)? 'RUTAS PENDIENTES POR CONDUCTOR '.$k : 'RUTAS PENDIENTES SIN CONDUCTOR') ,'B');
		$pdf->Ln( 10 );
		$pdf->putLine();
...
function dameDatosRuta($con, $empresa, $alias){
	if( empty($_POST) ) {
		infoCerrarVentana('Para ver los listados hágalo desde la web.');
	}
	...
	generaLogDB('GET LISTADO POR CONDUCTOR',ibase_errmsg(), $sql);
	...
	if( $rs ){
		...
		while( $row = ibase_fetch_assoc($rs) ){
			...
			$arr[utf8_encode($row['NOMBRE_CONDUCTOR'])][] = array(
