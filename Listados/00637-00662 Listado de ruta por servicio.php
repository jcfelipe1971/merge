<?php
	...
	foreach( $arr as $k => $conductor) {
		...
		$pdf->MultiCellSibiling( 150, $h, (($k)? 'RUTAS PENDIENTES POR SERVICIO: "'.$k.'"' : 'RUTAS PENDIENTES SIN SERVICIO') ,'B');
		$pdf->Ln( 10 );
		$pdf->putLine();
...
generaLogDB('GET LISTADO POR ARTICULO',ibase_errmsg(), $sql);
...
if( $rs ){
	...
	while( $row = ibase_fetch_assoc($rs) ){
		...
		$arr[utf8_encode($row['ARTICULO'] .' - '. $row['ARTICULO_TITULO'])][] = array(
