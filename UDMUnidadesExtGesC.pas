...
type
  TDMUnidadesExtGesC = class(TDataModule)
  ...
  public
     ...
     Unidades: integer;
     Unidad: string;
     procedure FiltraLinea(aEmpresa, aEjercicio, aCanal: integer; aSerie, aTipo: string; aRig, aLinea: integer; aArticulo: string; aEstado: integer);
     procedure FiltraPrecio(Articulo: string; proveedor: integer; Factor: double);
     function DameUnidades: double;
  end;

var
  DMUnidadesExtGesC : TDMUnidadesExtGesC;
...
procedure TDMUnidadesExtGesC.DMUnidadesExtCreate(Sender: TObject);
begin
...
procedure TDMUnidadesExtGesC.FiltraLinea(aEmpresa, aEjercicio, aCanal: integer; aSerie, aTipo: string; aRig, aLinea: integer; aArticulo: string; aEstado: integer);
begin
  Empresa := aEmpresa;
...
procedure TDMUnidadesExtGesC.QMUnidadesExtAfterPost(DataSet: TDataSet);
begin
  TFIBTableSet(DataSet).Transaction.CommitRetaining;
end;

procedure TDMUnidadesExtGesC.QMUnidadesExtNewRecord(DataSet: TDataSet);
begin
...
function TDMUnidadesExtGesC.DameUnidades: double;
var
  i, Decimales : integer;
  aFieldName : string;
  Medida : double;
begin
  ...
  if (Result < ud_min_venta) then
     Result := ud_min_venta;
  if (Result < ud_min_compra) then
     Result := ud_min_compra;
end;

procedure TDMUnidadesExtGesC.FiltraPrecio(Articulo: string; proveedor: integer; Factor: double);
begin
  ...
  begin
     ...
     begin
        try
           ...
           SQL.Add(' :EMPRESA, :EJERCICIO, :CANAL, :PROVEEDOR, ');
           SQL.Add(' :ARTICULO, :FECHA, :UNIDADES,:UNIDADES_EXT, ');
           SQL.Add(' :PRECIOP, :MONEDA, 0, :FACTOR) ');
...
function TDMUnidadesExtGesC.VerificaMultiplo(Numero: double; UnidadExt: integer): double;
var
