...
type
  ...
  THttpConnectionWinInet = class(TObject) // class(TInterfacedObject, IHttpConnection)
  private
     FAcceptTypes: string;
...
begin
  ...
  try
     while (Len > 0) and
{$IFDEF UNICODE}(CharInSet(Buffer[Len - 1], [#0..#32, '.'])) {$ELSE}
...
begin
  ...
  // Esto espera Key y Value separados por '=', pero ResponseHeader esta separado por ':'
  // Result := FResponseHeader.Values[Header];

  // Recorro las lineas del ResponseHeader
  i := 0;
  while ((i <= FREsponseHeader.Count - 1) and (i >= 0)) do
  begin
     // Comparo el comienzo al Header buscado
     if (Copy(FREsponseHeader[i], 1, Length(Header) + 1) = Header + ':') then
     begin
        Result := Copy(FREsponseHeader[i], Length(Header) + 2, Length(FREsponseHeader[i]));
        i := -1;
     end
     else
        Inc(i);
...
procedure THttpConnectionWinInet.Patch(AUrl: string; AContent, AResponse: TStream);
begin
...
procedure THttpConnectionWinInet.DoRequest(sMethod, AUrl: string; AContent, AResponse: TStream);
var
  iNetworkHandle, iConnectionHandle, iRequestHandle : HINTERNET;
...
begin
  ...
  if InternetCrackUrl(PChar(AUrl), Length(AUrl), 0, pURIC) then
  begin
     ...
     try
        if Assigned(iNetworkHandle) then
        begin
           ...
           try
              if Assigned(iConnectionHandle) then
              begin
                  ...
                  begin
                    ...
                    InternetSetOption(iRequestHandle, INTERNET_OPTION_PROXY_PASSWORD, PChar(FProxyCredentials.Password),
                      Length(FProxyCredentials.Password));
                       ...
                       begin
                          SetRequestHeader(FHeaders.Names[i], FHeaders.Values[FHeaders.Names[i]] {FHeaders.ValueFromIndex[i]});
                       end;
                       ...
                       repeat
                          ...
                          if HTTPSendRequest(iRequestHandle, nil, 0, PAnsiString(AData), Length(AData)) then
                          begin
                             iStatusIndex := 0;
                             iStatusLength := SizeOf(iStatus);
                             if HttpQueryInfo(iRequestHandle, HTTP_QUERY_STATUS_CODE or HTTP_QUERY_FLAG_NUMBER, @iStatus, iStatusLength, iStatusIndex) then
                             begin
                                ...
                                begin
                                   ...
                                   if HttpQueryInfo(iRequestHandle, HTTP_QUERY_STATUS_TEXT, @sStatusText[1], iStatusLength, iStatusIndex) then
                                   begin
                             ...
                             begin
                                while InternetReadFile(iRequestHandle, @arBuf, BUFSIZE, iBytesRead) and (iBytesRead > 0) do
                                begin
                             ...
                             begin
                                case GetLastError of
                                   ERROR_INTERNET_SEC_CERT_CN_INVALID:
                                      raise Exception{EHTTPVerifyCertError}.Create('SSL certificate common name (host name field) is incorrect.');
                                   ERROR_INTERNET_SEC_CERT_DATE_INVALID:
                                      raise Exception{EHTTPVerifyCertError}.Create('SSL certificate date that was received from the server is bad. The certificate is expired.');
                        {$IFNDEF DELPHI_7}
                                   12057{ERROR_INTERNET_SEC_CERT_REV_FAILED}:
                                      raise Exception{EHTTPVerifyCertError}.Create('Unable to validate the revocation of the SSL certificate because the revocation server is unavailable');
                        {$ENDIF}
