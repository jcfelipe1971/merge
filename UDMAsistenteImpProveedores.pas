unit UDMAsistenteImpProveedores;
...
type
  TDMAsistenteImpProveedores = class(TDataModule)
     TLocal: THYTransaction;
     TUpdate: THYTransaction;
     QImportaProveedor: THYFIBQuery;
...
var
  DMAsistenteImpProveedores : TDMAsistenteImpProveedores;

implementation

uses UDMMain, UEntorno, Variants, UUtiles, Dialogs, StrUtils, UHojaCalc, UFMAsistenteImpProveedores, FIBDataSetRW;

{$R *.dfm}

procedure TDMAsistenteImpProveedores.DataModuleCreate(Sender: TObject);
begin
...
procedure TDMAsistenteImpProveedores.DataModuleDestroy(Sender: TObject);
begin
  Filas.Free;
end;

function TDMAsistenteImpProveedores.LeerDatos(Archivo, Hoja: string): boolean;
var
  HCalc : THojaCalc;
  f, c, x, y : integer;
  ...
  try
     ...
     try
        ...
        try
           ...
           FMAsistenteImpProveedores.ProgresoImportacion.Min := 0;
           FMAsistenteImpProveedores.ProgresoImportacion.Max := y;
           ...
           begin
              ...
              FMAsistenteImpProveedores.ProgresoImportacion.Position := c;
           end;
...
function TDMAsistenteImpProveedores.DameFilas: TObjectList;
begin
  Result := Filas;
end;

function TDMAsistenteImpProveedores.DameColumnas: TStrings;
begin
  Result := (Filas.Items[0] as TStrings);
end;

procedure TDMAsistenteImpProveedores.RellenaCampos(Campos: TStrings);
var
...
begin
  ...
  try
     ...
     begin
        ...
        SelectSQL.Text := 'SELECT * FROM VER_PROVEEDORES_IMPORTACION WHERE EMPRESA = 0';
        Open;
...
procedure TDMAsistenteImpProveedores.InicializaImportacion(aSeparadorDecimal: char);
var
...
begin
  ...
  try
     DMAsistenteImpProveedores.RellenaCampos(ListaCampos);
  ...
  with QImportaProveedor do
  begin
     SQL.Clear;
     SQL.Add(' INSERT INTO VER_PROVEEDORES_IMPORTACION');
     SQL.Add(' (' + Campos + ')');
     SQL.Add(' VALUES ');
     SQL.Add(' (' + Valores + ')');
  end;

  DMMain.Log(QImportaProveedor.SQL.Text);
end;

procedure TDMAsistenteImpProveedores.NuevoProveedor;
var
  i : integer;
begin
  DMMain.Log('NuevoProveedor');
  with QImportaProveedor do
  begin
...
procedure TDMAsistenteImpProveedores.AsignaParametro(Parametro, Valor: string);
var
  i, j : integer;
  s : string;
begin
     ...
     for i := 0 to Params.Count - 1 do
        ...
        begin
           ...
           begin
              ...
              begin
                 FMAsistenteImpProveedores.Log(Format(_('Se reduce el tamaño del campo %s. (Max. = %d). Se importa: %s'), [Params[i].Name, Params[i].Size, Copy(Valor, 1, Params[i].Size)]));
                 FMAsistenteImpProveedores.Log('');
              end;
...
procedure TDMAsistenteImpProveedores.CrearProveedor;
var
  i : integer;
begin
  // Crea el proveedor, rellenando algunos parametros si fuera necesario.
  DMMain.Log('CrearProveedor');
  with QImportaProveedor do
  begin
     for i := 0 to Params.Count - 1 do
     begin
...
procedure TDMAsistenteImpProveedores.ActualizaDatosImportacion(aFileName: string; var aLastRow, aLastCol, aCountSheet: integer; var aActiveSheetName: string; Sheets: TStrings);
var
  i : integer;
begin
  aLastRow := 0;
