...
type
  TDMOfertas = class(TDataModule)
     ...
     QMNIFNOTAS: TMemoField;
     xProBuscaEscandalloID_ESC: TIntegerField;
     xProBuscaEscandalloESCANDALLO: TIntegerField;
     xProBuscaEscandalloCOMPUESTO: TFIBStringField;
     xProBuscaEscandalloTITULO: TFIBStringField;
     xProBuscaEscandalloESTADO: TIntegerField;
     xProBuscaEscandalloTIPO: TFIBStringField;
     xProBuscaEscandalloTIPO_ID_ESC: TFIBStringField;
     ...
     QMCabeceraFECHA_VALIDEZ: TDateTimeField;
     ...
     QMDetalleID_OFERTA_ESC: TIntegerField;
     ...
     QMCabeceraAUTORIZADO: TIntegerField;
     ...
     QMCabeceraPREPARADO: TIntegerField;
     QMCabeceraDIR_ENTREGA_MANUAL: TMemoField;
     QMCabeceraACEPTADA: TIntegerField;
     ...
     QMDetalleTIPO_ID_ESC: TFIBStringField;
     QMDetalleENTRADA_AGRUPACION: TIntegerField;
     QMCabeceraFECHA_ENTREGA_MANUAL: TDateTimeField;
     QMCabeceraEMPAQUETADO_EMPRESA: TIntegerField;
     QMCabeceraDIR_RECOGIDA_MANUAL: TMemoField;
     QMCabeceraEQUIPO_RECOGIDA: TFIBStringField;
     QMCabeceraEQUIPO_ENTREGA: TFIBStringField;
     QMCabeceraMATRICULA_RECOGIDA: TFIBStringField;
     QMCabeceraMATRICULA_ENTREGA: TFIBStringField;
     ...
     QMCabeceraRECHAZADA: TIntegerField;
     QMCabeceraRECHAZADA_MOTIVO: TFIBStringField;
     QMCabeceraRECHAZADA_FECHA: TDateTimeField;
     ...
     procedure QMNIFPAISChange(Sender: TField);
     ...
     procedure QMCabeceraDIR_NOMBRE_2_KRIGetText(Sender: TField; var Text: string; DisplayText: boolean);
     ...
     procedure xClienteBeforeOpen(DataSet: TDataSet);
     ...
     procedure QMCabeceraRECHAZADAChange(Sender: TField);
  private
     ...
     GEST_OFERTAS_ESC: boolean;
     GEST_ESC_PORDEFECTO: boolean;
     KriConf126, KriConf462: boolean;
     OldLinea: TStrings;
     gUnidades: integer;
     ...
     Param_DOCREFE001: boolean;
     Param_CLIREFE001: boolean;
     Param_OFCACEP001: boolean;
     Param_OFCPRES001: boolean;
     Param_DOCVPRY001: integer;
     Param_OFCCLIE001: integer;
  ...
  public
     ...
     procedure VersionesDeOFCKri;
     procedure AbrirCerrar(Estado: integer);
     function PuedeHacerPedidoOferta: boolean;
     procedure InsertarOferta(id_s: integer; AgregarAlFinal, MantenerPrecioYDescuentos: boolean);
     function ExisteRango(Rango: integer): boolean;
     procedure HabilitarBotonesEsc;
     procedure CambiaDatosEsp;
     // procedure HabilitarBotones;
     procedure CalculaComisionKRI20; //SFG_JLA
     function BuscaOferta(id_detalles_s: integer): integer;
     procedure OrdenarLineasPorDireccionEntrega;
     procedure CrearOfertaCrm(IDAccion, IDContacto, Cliente, Agente: integer);
     procedure CrearOfertaHubspot(Cliente: integer);
     procedure BusquedaArticulo(Descripcion: string);
     procedure RellenaDatosOldLinea;
     procedure AsignarImagenLinea;
     procedure DuplicarLinea;
     procedure Despiece;
     function GeneraReparacion(Serie: string; Fecha: TDateTime): integer;
     function DameIDREparacionOferta(id_s: integer): integer;
     procedure AsignaEmpleados;
     procedure CreaReciboAnticipo;
     procedure ActualizaPorteSegunZona;
     function MargenMinimoSuperado: boolean;
     ...
     procedure ImportaDetalleDesdeExcel(Archivo: string);
     procedure InsertaArticulosCliente;
     procedure RecuperarCostesEscandallo;
...
uses UDMMain, UUtiles, UDameDato, UEntorno, {UFROfertasC,} UFMOfertas, URecursos,
  uFBusca, UFMUnidadesExtGes, UFMPreciosArticuloDoc, UFInfoHistorico, uFParada,
  UFMAlbaranes, UFMPedidos, UFMFacturas, UFMGarantiasKri, UFOrdenaLineas, UFMOfertasTallas,
  UFMain, UCrmDMContactos, UFSendCorreo, ZUFMOfertasArtDet, UFMDespiece, UParam, UFMReciboNuevo,
  UFMAsignaEmpleado, Math, UHojaCalc, DateUtils, UFMTicketVenta, UImagenes;

{$R *.DFM}

procedure TDMOfertas.DMDocsOfertaCCreate(Sender: TObject);
var
  i : integer;
begin
  ...
  Kriconf462 := (DMMain.EstadoKri(462) = 1);
  ...
  begin
     Clear;
     Add(' SELECT * FROM VER_CABECERAS_OFERTA_C ');
     Add(' WHERE  ');
  ...
  begin
     try
        ...
        SQL.Text := 'SELECT GEST_OFERTAS_ESC, GEST_ESC_PORDEFECTO FROM PRO_CONSTANTES';
        ExecQuery;
        GEST_OFERTAS_ESC := (FieldByName['GEST_OFERTAS_ESC'].AsInteger = 1);
        GEST_ESC_PORDEFECTO := (FieldByName['GEST_ESC_PORDEFECTO'].AsInteger = 1);
        FreeHandle;
     finally
        Free;
     end;
...
procedure TDMOfertas.DataModuleDestroy(Sender: TObject);
begin
...
procedure TDMOfertas.Graba(DataSet: TDataSet);
begin
  DataSet.Refresh;
  InfoActualiza;
...
procedure TDMOfertas.QMDetalleNewRecord(DataSet: TDataSet);
var
  i : integer;
...
begin
  ...
  QMDetalleID_IMAGEN.AsInteger := 0;
...
procedure TDMOfertas.QMCabeceraNewRecord(DataSet: TDataSet);
begin
  ...
  QMCabeceraALMACEN.AsString := DMMain.DameAlmacenDocumento('OFC', Serie);
  QMCabeceraFECHA_ENTREGA_PREV.AsDateTime := QMCabeceraFECHA.AsDateTime;
  QMCabeceraTARIFA.AsString := REntorno.TarifaDefecto;
  QMCabeceraPROYECTO.AsInteger := Param_DOCVPRY001;
  QMCabeceraACEPTADA.AsInteger := BoolToInt(Param_OFCACEP001);
  QMCabeceraPRESUPUESTO.AsInteger := BoolToInt(Param_OFCPRES001);
  QMCabeceraCAMPANYA.AsInteger := 0;
  QMCabeceraPOR_FINANCIACION.AsFloat := 0;
  QMCabeceraID_S.AsInteger := 0;
  QMCabeceraCONTACTO.AsInteger := 0; {dji lrk kri}
  ...
  QMCabeceraAUTORIZADO.AsInteger := 0;
  QMCabeceraDOCUMENTO_MUESTRAS.AsInteger := 0;
  QMCabeceraPEDIDO_CLIENTE.AsString := '';
  QMCabeceraPREPARADO.AsInteger := 0;
  QMCabeceraRECHAZADA_FECHA.AsDateTime := Now;
  Editando := False;
  Tarifa_old := '';

  if (Param_OFCCLIE001 <> 0) then
     QMCabeceraCLIENTE.AsInteger := Param_OFCCLIE001;
...
procedure TDMOfertas.QMCabeceraCLIENTEChange(Sender: TField);
begin
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Text := 'SELECT FIRST 1 CONTACTO FROM SYS_TERCEROS_CONTACTOS WHERE TERCERO = :TERCERO ORDER BY CONTACTO';
...
procedure TDMOfertas.QMCabeceraAGENTEChange(Sender: TField);
begin
  with xAgentes do
  begin
     Close;
...
procedure TDMOfertas.QMCabeceraDIRECCIONChange(Sender: TField);
begin
...
procedure TDMOfertas.QMCabeceraBeforePost(DataSet: TDataSet);
var
  DS : TFIBDataSet;
  T : TFIBTransaction;
  Documentos : string;
begin
  ...
  if Param_DOCREFE001 and (QMCabeceraSU_REFERENCIA.AsString <> '') then
  begin
  ...
  begin
     ...
     begin
        ...
        if (Documentos > '') then
           ShowMessage(format('Hay otro/s documento/s con el mismo nro. de pedido para este cliente.'#13#10 + '%s', [Documentos]));
     end;
  end;
  ...
  begin
     ...
     {dji lrk kri - Tomo las notas segun idioma - Iberfluid}
     if (QMCabeceraIDIOMA.AsString > ' ') then
     begin
        ...
        try
           ...
           begin
              ...
              if (not Transaction.InTransaction) then
                 Transaction.StartTransaction;
              SelectSQL.Text := 'SELECT NOTAS_KRI,NOTAS_INTERNAS_KRI FROM SYS_IDIOMAS WHERE IDIOMA=?IDIOMA';
              Params.ByName['IDIOMA'].AsString := QMCabeceraIDIOMA.AsString;
              Open;
              QMCabeceraNOTAS.Assign(FieldByName('NOTAS_KRI'));
              QMCabeceraNOTAS_INTERNAS_KRI.Assign(FieldByName('NOTAS_INTERNAS_KRI'));
              Close;
              Transaction.Commit;
           end;
        finally
...
procedure TDMOfertas.QMDetalleARTICULOChange(Sender: TField);
var
...
begin
  ...
  if (not DMMain.ArticuloBloqueado(Sender.AsString, 'OFC')) then
  begin
     ...
     begin
        // Nomes entrem al proces en els components que no tenen escandall especial
     ...
     begin
        try
           ...
           QMDetalleTIPO_IMPUESTO_ADICIONAL.AsString := FieldByName['TIPO_IMPUESTO_ADICIONAL'].AsString;
...
procedure TDMOfertas.InfoActualiza;
begin
...
procedure TDMOfertas.QMDetalleBeforePost(DataSet: TDataSet);
var
  DS : TFIBDataSetRO;
  s : string;
...
begin
  ...
  {dji lrk kri - Que muestre las notas del articulo cuando se inserta.}
  if ((DataSet.State = dsInsert) and (DataSet.Name = 'QMDetalle')) then
  begin
     if (DMMain.EstadoKri(168) = 1) then
     begin
        if (Trim(QMDetalleARTICULO.AsString) > '') then
        begin
           ...
           try
              ...
              begin
                 ...
                 Transaction := TLocal;
                 Close;
                 SelectSQL.Text := 'SELECT NOTAS FROM ART_ARTICULOS WHERE EMPRESA=' + REntorno.EmpresaStr + ' AND ARTICULO=''' + QMDetalleARTICULO.AsString + '''';
                 Open;
                 s := FieldByName('NOTAS').AsString;
                 // FreeHandle ;
              end;
           finally
              FreeAndNil(DS);
  ...
  {TOLYCOR}
  if ((DataSet.State = dsInsert) and Param_MODREST015) then
  begin
...
procedure TDMOfertas.QMCabeceraAfterOpen(DataSet: TDataSet);
begin
  xMonedas.Open;
  xCliente.Open;
  xAlmacenes.Open;
...
procedure TDMOfertas.QMCabeceraALMACENChange(Sender: TField);
begin
...
procedure TDMOfertas.AntesDeEditar(DataSet: TDataSet);
begin
...
function TDMOfertas.MargenMinimoSuperado: boolean;
var
  MargenMinimo : double;
  id : integer;
  PermiteModificar : string;
begin
  /// Verifica si todas las lineas se estan vendiendo con un margen minimo
  /// No se tiene en cuenta ARTICULOS_VARIOS
  /// *** SI SE VENDE POR DEBAJO DE MARGEN PERO HAY UN PEDIDO QUE TAMBIEN LO HACE, DEJAMOS SEGUIR
  /// Establecido mediante parámetro DOCMARG001

  Result := True;
  MargenMinimo := StrToFloatDef(LeeParametro('DOCMARG001', Serie), -1);

  if (MargenMinimo >= 0) then
  begin
     PermiteModificar := LeeParametro('DOCMARG002', Serie);

     if (PermiteModificar = 'N') then
     begin
        ...
        begin
           ...
           try
              First;
              // Mientras los margenes se superen recorro las lineas exceptuando "Articulos Varios".
              while (not EOF) and Result do
              begin
                 {
                 if ((DMOfertas.QMDetalleARTICULO.AsString <> REntorno.ArtTextoLibre) and (MargenMinimo > DMOfertas.QMDetalleMARGEN.AsFloat)) then
                    Result := False;
                 }

                 // Se verifica si se vende por debajo del margen.
                 // Si se vende por debajo de margen pero hay un pedido que tambien lo hace, dejamos seguir
                 with THYFIBQuery.Create(nil) do
                 begin
                    try
                       Close;
                       DataBase := DMMain.DataBase;
                       SQL.Text := 'SELECT PUEDE FROM OFC_SUPERA_MARGEN_MINIMO(:ID_DETALLES_S, :MARGEN_MINIMO, :MARGEN)';
                       Params.ByName['ID_DETALLES_S'].AsInteger := QMDetalleID_DETALLES_S.AsInteger;
                       Params.ByName['MARGEN_MINIMO'].AsDouble := MargenMinimo;
                       Params.ByName['MARGEN'].AsDouble := QMDetalleMARGEN.AsFloat;
                       ExecQuery;
                       Result := (FieldByName['PUEDE'].AsInteger = 1);
                       FreeHandle;
                    finally
                       Free;
                    end;
        ...
        // Si todo supera margenes reposiciono en la linea en la que estaba.
        if Result then
...
procedure TDMOfertas.TraspasarOfertaActual(TipoDestino: string);
var
  id_s_destino : integer;
  Seguir : boolean;
begin
  if not MargenMinimoSuperado then
     ShowMessage(_('No se ha superado el margen minimo de venta.'))
  else
  begin
     Seguir := True;
     if (QMCabeceraFORMA_PAGO.AsString <> xCliente.FieldByName('FORMA_PAGO').AsString) then
        Seguir := (MessageDlg(format(_('La forma de pago del documento es distinta de la forma de pago habitual del cliente (%s). ¿Desea seguir?'), [xCliente.FieldByName('FORMA_PAGO').AsString]), mtConfirmation, [mbYes, mbNo], 0) = mrYes);
     ...
     begin
        ...
        begin
           try
              ...
              SQL.Add(' EXECUTE PROCEDURE G_TRASPASO_OFERTA_A_OTRO_MUL ');
              SQL.Add(' (:EMPRESA, :EJERCICIO, :CANAL, :SERIE, :TIPO, :RIG, ');
              SQL.Add(' :D_TIPO, :ENTRADA, :FECHA, :D_EJERCICIO, :D_CANAL, :D_SERIE, ');
              SQL.Add(' :ID_S, 0, 0, 0) ');

              // Datos de la oferta a traspasar
              Params.ByName['EMPRESA'].AsInteger := QMCabeceraEMPRESA.AsInteger;
              ...
              // Datos del documento destino a generar
              Params.ByName['D_TIPO'].AsString := TipoDestino; // 'PEC', 'ALB', 'FAC'
              ...
              Params.ByName['FECHA'].AsDateTime := REntorno.FechaTrab; // Debe estar dentro del ejercicio destino
              ...
              id_s_destino := FieldByName['ID_S_D'].AsInteger;
        ...
        {TOLYCOR}
        // Es fa el traspas de l'escandall d'ofertes de ventes a les comandes,
        // sempre que el check de configuración a gestión, estigui marcat
        if (GEST_OFERTAS_ESC and (TipoDestino = 'PEC')) then
        begin
           with THYFIBQuery.Create(nil) do
           begin
              try
                 Close;
                 DataBase := DMMain.DataBase;
                 SQL.Text := 'EXECUTE PROCEDURE PRO_TRASPASO_OFERTASV_ESC(:ID_OFERTAS_V, :ID_NUEVO_PEDIDO, :ENTRADA)';
                 Params.ByName['ID_OFERTAS_V'].AsInteger := QMDetalleID_S.AsInteger;
                 Params.ByName['ID_NUEVO_PEDIDO'].AsInteger := id_s_destino;
                 Params.ByName['ENTRADA'].AsInteger := REntorno.Entrada;
                 ExecQuery;
                 FreeHandle;
              finally
        ...
        begin
           try
              ...
              while not EOF do
              begin
                 ReiniciaStock(FieldByName('ARTICULO').AsString);
                 Next;
              end;
              First;
           finally
        ...
        begin
           QMCabecera.Refresh;
           AbreDocumentos(id_s_destino, TipoDestino, QMCabeceraSERIE.AsString);
        end
        else
        begin
           QMCabecera.Close;
...
procedure TDMOfertas.QMCabeceraFORMA_PAGOChange(Sender: TField);
begin
  with xFormasPago do
  begin
     Close;
...
procedure TDMOfertas.QMCabeceraCAMPANYAChange(Sender: TField);
begin
  with xCampanyas do
  begin
     Close;
...
function TDMOfertas.BusquedaCompleja: integer;
begin
  with QMCabecera do
  begin
     ...
     begin
        ...
        SelectSQL.Add(' AND (EJERCICIO = ?EJERCICIO OR ((EJERCICIO < ?EJERCICIO) AND (ESTADO = 0))) ');
...
procedure TDMOfertas.Duplica(Relaciona: integer);
var
...
begin
  ...
  // Si guardo la relación, entonces cierro el origen
  if (Relaciona = 1) then
     AbrirCerrar(5);
...
procedure TDMOfertas.ActualizaCondicionesCli(CrearCondicionModelo: boolean);
begin
...
procedure TDMOfertas.QMCabeceraPostError(DataSet: TDataSet; E: EDatabaseError; var Action: TDataAction);
begin
...
procedure TDMOfertas.QMCabeceraAfterPost(DataSet: TDataSet);
begin
  Graba(DataSet);
  {
dji lrk kri - No recalculo precios si cambia la moneda o la fecha
  ...
  { TODO -oDuilio : Garantias Navarrete. Filtrar comportamiento con KRICONF_XXX }
  {dji lrk kri ------------------------------------------------------------------}
  {debe mirar si hay garantias si la serie es de reparaciones. Navarrete         }
  if (Trim(serie) = 'RP') then
  begin
     AbreForm(TFMGarantiasKri, FMGarantiasKri);
     FMGarantiasKri.Muestra(QMCabeceraID_S.AsInteger);
     QMDetalle.First;
     if (QMDetalleARTICULO.AsString <> '000H2') then
     begin
        QMDetalle.Append;
        QMDetalleARTICULO.AsString := '000H2';
        QMDetalleUNIDADES.AsInteger := 0;
        QMDetalle.Post;
     end;
  end;
...
begin
  ...
  param_OFCNOST001 := (LeeParametro('OFCNOST001', Serie) = 'S');
  ...
  begin
     ...
     if (LeeParametro('OFCULTD001', Serie) = 'S') then
        Last;
  end;
end;

procedure TDMOfertas.PreparaNotasDetalle;
begin
  if (QMDetalle.State in [dsEdit, dsInsert]) then
     QMDetalle.Post;
...
procedure TDMOfertas.RefrescaDetalle;
begin
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Add(' EXECUTE PROCEDURE UT_REFRESCA_DETALLE_S( ');
           SQL.Add(' :EMPRESA, :EJERCICIO, :CANAL, :SERIE, :TIPO, :RIG, ');
           SQL.Add(' :CLIENTE, :TARIFA, :FECHA, :MONEDA) ');
           Params.ByName['EMPRESA'].AsInteger := QMCabeceraEMPRESA.AsInteger;
           Params.ByName['EJERCICIO'].AsInteger := QMCabeceraEJERCICIO.AsInteger;
           Params.ByName['CANAL'].AsInteger := QMCabeceraCANAL.AsInteger;
           Params.ByName['SERIE'].AsString := QMCabeceraSERIE.AsString;
...
procedure TDMOfertas.CambiaUnidadesExt(Articulo: TField; Modo, Mostrar: boolean);
var
  Tarifa : string;
...
procedure TDMOfertas.CambiaPreciosTarifa;
var
...
begin
  ...
  Precio := DevuelvePrecio(1, QMDetalleARTICULO.AsString, QMDetalleALMACEN.AsString, Bueno,
...
procedure TDMOfertas.DatosStocks(var Articulo, Almacen: string);
begin
  Articulo := QMDetalleARTICULO.AsString;
  Almacen := QMCabeceraALMACEN.AsString;
end;

procedure TDMOfertas.Historico;
begin
...
procedure TDMOfertas.BorraLineaUnidadesExt;
begin
  if QMDetalle.State = dsInsert then
  begin
     with THYFIBQuery.Create(nil) do
...
procedure TDMOfertas.DatosHerencia(var rig, Ejercicio: integer; var tipo, su_referencia, titulo, serie: string);
begin
...
procedure TDMOfertas.EstaleceCondiciones(Articulo: TField; Precio: double; Forzar: boolean = False);
var
  Modo : integer;
begin
  DMMain.LogIni('TDMOfertas.EstaleceCondiciones');
     ...
     begin
        ...
        begin
           try
              Close;
              DataBase := DMMain.DataBase;
              SQL.Text := 'SELECT PRECIO FROM GES_DETALLES_S_PROVEEDORES WHERE ID_S = :ID_S AND ID_A = :ID_A AND UTILIZAR_COSTE = 1';
...
procedure TDMOfertas.QMDetalleUNIDADESChange(Sender: TField);
var
  FactorUnidad : double;
begin
...
procedure TDMOfertas.QMDetalleAfterOpen(DataSet: TDataSet);
begin
  xArticulos.Open;
  RellenaDatosLinea(OldLinea);
end;

function TDMOfertas.Traspasado: boolean;
begin
...
procedure TDMOfertas.RefrescaTabla;
begin
  if (QMCabecera.RecordCount > 0) then
  begin
...
procedure TDMOfertas.AnulaDocumento;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMOfertas.QMCabeceraMONEDAChange(Sender: TField);
begin
  xMonedas.Close;
  xMonedas.Open;
end;

procedure TDMOfertas.CambiaMonedaDetalle;
begin
...
procedure TDMOfertas.MascarasMoneda;
begin
  ...
  xInfoActualizadaB_DTO_LINEAS.DisplayFormat := LocalMascaraL;
...
procedure TDMOfertas.QMCabeceraTOTAL_CANALGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := FormatFloat(DMMain.MascaraMoneda(QMCabeceraMONEDA.AsString, 1),
     QMCabeceraLIQUIDO.AsFloat);
end;

procedure TDMOfertas.QMCabeceraTARIFAChange(Sender: TField);
begin
  xTarifas.Close;
  xTarifas.Open;
  EntornoDoc.Tarifa := QMCabeceraTARIFA.AsString;
...
procedure TDMOfertas.FiltraCabecera(Ejercicio: integer; Serie: string; Rig: integer);
begin
  with QMCabecera do
  begin
     Close;
     SelectSQL.Clear;
     SelectSQL.Add(' SELECT * FROM VER_CABECERAS_OFERTA_C ');
     SelectSQL.Add(' WHERE ');
     SelectSQL.Add(' EMPRESA = ?EMPRESA AND ');
...
function TDMOfertas.HayDocumentos: boolean;
begin
  Result := (QMCabecera.FieldByName('RIG').AsInteger <> 0);
end;

function TDMOfertas.HayDetalle: boolean;
begin
  //Result := (QMDetalle.FieldByName('LINEA').AsInteger <> 0);
  Result := (QMDetalle.RecordCount > 0);
end;

procedure TDMOfertas.QMCabeceraAfterInsert(DataSet: TDataSet);
begin
  monedaold := QMCabeceraMONEDA.AsString;
end;

procedure TDMOfertas.ControlaRestriccion;
begin
  ControlaBorrado(QMCabeceraID_S.AsInteger, QMCabeceraTIPO.AsString);
end;

procedure TDMOfertas.QMCabeceraIDIOMAChange(Sender: TField);
begin
  with xIdiomas do
  begin
     Close;
...
{No se utiliza
procedure TDMOfertas.CambiaMonedaFecha;
var
  Q : THYFIBQuery;
begin
...
procedure TDMOfertas.QMCabeceraAfterScroll(DataSet: TDataSet);
begin
  IdDocEliminaPorte := 0;
  fecha_old := QMCabeceraFECHA.AsDateTime;
end;

procedure TDMOfertas.AbreDocumentos(id_s_d: integer; destino, serie: string);
begin
  if (destino = 'PEC') then
     TraspasoAPedido(Serie, id_s_d)
  else
  if (destino = 'ALB') then
...
function TDMOfertas.DameDatosClientes: boolean;
begin
...
procedure TDMOfertas.GrabaDatosCliente(Estado: integer);
begin
...
procedure TDMOfertas.QMNIFNewRecord(DataSet: TDataSet);
begin
...
procedure TDMOfertas.QMNIFPAISChange(Sender: TField);
begin
end;

{No se utiliza
function TDMOfertas.DameEstado: smallint;
begin
...
procedure TDMOfertas.QMCabeceraBeforeInsert(DataSet: TDataSet);
begin
  with EntornoDoc do
  begin
     Empresa := REntorno.Empresa;
...
procedure TDMOfertas.OrdenarLineas;
begin
  with QMDetalle do
  begin
     DisableControls;
     try
        OrdenaLineas('OFC', QMCabeceraID_S.AsInteger, Self);
        Close;
        Open;
     finally
        EnableControls;
...
procedure TDMOfertas.OrdenarLineasPorDireccionEntrega;
(*
var
  Q : THYFIBQuery;
  DS : TFIBDataSet;
  ...
  try
     ...
     begin
        ...
        try
           ...
           SelectSQL.Add(' JOIN GES_DETALLES_S_OFC O ');
           SelectSQL.Add(' ON D.ID_DETALLES_S = O.ID_DETALLES_S ');
           SelectSQL.Add(' WHERE D.ID_S = ?ID_S ');
           SelectSQL.Add(' ORDER BY O.DIRECCION_ENTREGA, D.ARTICULO, D.LINEA ');
           Params.ByName['ID_S'].AsInteger := QMCabeceraID_S.AsInteger;
           Open;
           while not EOF do
           begin
...
procedure TDMOfertas.QMDetalleAfterPost(DataSet: TDataSet);
var
...
begin
  // Graba(DataSet);
  { ESTO ES DE PRODUCCION SOFINTER, NO SOLO DE TolYCor}
  { Esto debe hacerse por trigger.
  if (LeeParametro('MODREST006') = 'S') then
  begin
     //sfg.albert - S'assigna l'escandall per defecte o oferta d'aquell compost, si existeix
     Q := THYFIBQuery.Create(nil);
     try
        with Q do
        begin
           ...
           Params.ByName['EMPRESA'].AsInteger := REntorno.Empresa;
           Params.ByName['ARTICULO'].AsString := QMDetalleARTICULO.AsString;
           if (not GEST_OFE_ESC) then
              Params.ByName['TIPO'].AsString := 'OFE'
           else
     ...
     if ((IDEscandallo <> 0) and GEST_ESC_PORDEFECTO) then
     begin
        Q := THYFIBQuery.Create(nil);
        try
           with Q do
           begin
              Close;
              DataBase := DMMain.DataBase;
              SQL.Add('UPDATE GES_DETALLES_S_OFC ');
              SQL.Add('SET ');
              SQL.Add('ID_ESC = ?ID_ESCANDALLO, ');
              SQL.Add('TIPO_ID_ESC = ?TIPO_DEFECTO ');
              SQL.Add('WHERE ID_DETALLES_S = ?ID_DETALLES ');
              Params.ByName['ID_DETALLES'].AsInteger := QMDetalleID_DETALLES_S.AsInteger;
              Params.ByName['ID_ESCANDALLO'].AsInteger := IDEscandallo;
              if (not GEST_OFE_ESC) then
                 Params.ByName['TIPO_DEFECTO'].AsString := 'OFE'
              else
                 Params.ByName['TIPO_DEFECTO'].AsString := 'EPR';
              ExecQuery;
              FreeHandle;
           end;
        finally
     ...
     // Habilito o deshabilito el botón de la OF, depenent de si té escandall o no
     // Se realiza en FMOfertas.HabilitaBotonesDetalle HabilitarBotones;
  end;
  }
...
procedure TDMOfertas.AjustaPorteMinimo;
var
...
procedure TDMOfertas.CierraAbreDet;
begin
  Refrescar(QMDetalle, 'ID_DETALLES_S', QMDetalleID_DETALLES_S.AsInteger);
end;

procedure TDMOfertas.QMDetalleAfterDelete(DataSet: TDataSet);
begin
  Graba(DataSet);
...
procedure TDMOfertas.QMDetalleBeforeDelete(DataSet: TDataSet);
begin
  DMMain.RestingeEdicion(DataSet, QMCabeceraESTADO.AsInteger);
  ConPromociones := TienePromociones(QMDetalleID_DETALLES_S.AsInteger);
end;

procedure TDMOfertas.RefrescaCabecera(Accion: integer);
begin
  case Accion of
...
procedure TDMOfertas.QMDetallePRECIOChange(Sender: TField);
begin
  CalculaComisionKRI20;
end;

procedure TDMOfertas.QMDetalleP_COSTEChange(Sender: TField);
begin
  CalculaComisionKRI20;
end;

procedure TDMOfertas.QMDetalleCOMISIONChange(Sender: TField);
begin
  // Utiliza precio coste de art_tarifas_precios
  if (param_DOCPCTA001) then
  begin
...
procedure TDMOfertas.CalculaComisionKRI20;
begin
...
{TOLYCOR}
procedure TDMOfertas.CambiaDatosEsp;
var
  NoEditable : boolean;
  TipoArticulo : string;
  ComCorte : integer;
begin
  ...
  begin
     ...
     begin
        ...
        TZFMOfertasArtDet.Create(Self).AbreDatos(QMDetalleID_DETALLES_S.AsInteger,
...
procedure TDMOfertas.QMCabeceraCONTACTOChange(Sender: TField);
begin
...
procedure TDMOfertas.QMCabeceraPROYECTOChange(Sender: TField);
var
...
procedure TDMOfertas.BuscaArticulo(Articulo: string);
begin
...
procedure TDMOfertas.FiltraClienteKri(Ejercicio: integer; Serie: string; Cliente, Estado: integer);
begin
  with QMCabecera do
  begin
     Close;
...
procedure TDMOfertas.MoverLinea(direccion: integer);
var
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Text := 'EXECUTE PROCEDURE KRI_MUEVE_LINEA(' + QMDetalleID_DETALLES_S.AsString + ',' + IntToStr(direccion) + ')';
           ExecQuery;
           FreeHandle;
        finally
           Free;
     ...
     QMCabecera.Refresh;
     QMDetalle.First;
     while ((not QMDetalle.EOF) and (QMDetalleID_DETALLES_S.AsInteger <> id_detalles_s)) do
        QMDetalle.Next;
     QMDetalle.EnableControls;
  end;
end;

function TDMOfertas.Inhabilitar: boolean;
begin
  Result := ((QMDetalle.State = dsInsert) or (QMDetalle.State = dsEdit));
end;

function TDMOfertas.EstadoDocumento: integer;
begin
  Result := -1;
...
procedure TDMOfertas.AbreQMNif;
begin
...
procedure TDMOfertas.CierraQMNif;
begin
  QMNif.Close;
end;

procedure TDMOfertas.QMCabeceraBeforeDelete(DataSet: TDataSet);
begin
  DMMain.RestingeEdicion(DataSet, QMCabeceraESTADO.AsInteger);

  // Esto es para evitar un error raro al borrar la ultima oferta del dataset.
  xInfoActualizada.Close;
end;

procedure TDMOfertas.QMCabeceraDIR_NOMBRE_2_KRIGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := '';
  if (QMCabeceraDIRECCION.AsInteger > 0) then
  begin
     with THYFIBQuery.Create(nil) do
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Add(' SELECT DIR_NOMBRE_2 FROM SYS_TERCEROS_DIRECCIONES ');
           SQL.Add(' WHERE ');
           SQL.Add(' TERCERO=' + QMCabeceraTERCERO.AsString + ' AND DIRECCION=' + QMCabeceraDIRECCION.AsString);
           ExecQuery;
           Text := FieldByName['DIR_NOMBRE_2'].AsString;
           FreeHandle;
        finally
           Free;
        end;
...
procedure TDMOfertas.QMCabeceraTITULOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  // DMMain.LogIni('TDMOfertas.QMCabeceraTITULOGetText');
...
procedure TDMOfertas.VersionesDeOFCKri;
var
  s : string;
  DS : TFIBDataSet;
begin
  ...
  try
     ...
     begin
        ...
        try
           ...
           if (not Transaction.InTransaction) then
              Transaction.StartTransaction;
           SelectSQL.Text := 'select ''EMPRESA=''||REMPRESA||'' AND EJERCICIO=''||REJERCICIO||'' AND CANAL='' ||RCANAL||'' AND SERIE=''''''||RSERIE||'''''' AND RIG=''||RRIG ' +
              'from g_relaciones_origenes (?EMPRESA, ?EJERCICIO, ?CANAL, ?SERIE, ?TIPO, ?RIG) where RTIPO=''OFC'' ' +
              'union ' +
              'select ''EMPRESA=''||REMPRESA||'' AND EJERCICIO=''||REJERCICIO||'' AND CANAL='' ||RCANAL||'' AND SERIE=''''''||RSERIE||'''''' AND RIG=''||RRIG  ' +
              'from g_relaciones_destinos (?EMPRESA, ?EJERCICIO, ?CANAL, ?SERIE, ?TIPO, ?RIG) where RTIPO=''OFC'' ';
           Params.ByName['EMPRESA'].AsInteger := QMCabeceraEMPRESA.AsInteger;
           Params.ByName['EJERCICIO'].AsInteger := QMCabeceraEJERCICIO.AsInteger;
           Params.ByName['CANAL'].AsInteger := QMCabeceraCANAL.AsInteger;
           Params.ByName['SERIE'].AsString := QMCabeceraSERIE.AsString;
           ...
           Params.ByName['RIG'].AsInteger := QMCabeceraRIG.AsInteger;
           Open;
           s := '(empresa=' + QMCabeceraEMPRESA.AsString + ' and ejercicio=' + QMCabeceraEJERCICIO.AsString + ' and canal=' + QMCabeceraCANAL.AsString + ' and serie=''' + QMCabeceraSERIE.AsString + ''' and tipo=''OFC'' and rig=' + QMCabeceraRIG.AsString + ')';
           while not EOF do
           begin
              s := s + ' or ' + '(' + Fields[0].AsString + ')';
              Next;
           end;
           Close;
           Transaction.Commit;
  ...
  begin
     Close;
     SelectSQL.Text := 'SELECT * FROM VER_CABECERAS_OFERTA_C WHERE ' + s + ' ORDER BY EJERCICIO,RIG';
     Open;
  end;
end;

procedure TDMOfertas.AbrirCerrar(Estado: integer);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'UPDATE VER_CABECERAS_OFERTA_C SET ESTADO=?ESTADO WHERE ID_S=?ID_S AND ESTADO<>?ESTADO';
...
function TDMOfertas.PuedeHacerPedidoOferta: boolean;
var
...
{procedure TDMOfertas.QMDetalleAfterScroll(DataSet: TDataSet);
begin
   LiquidoAnt := QMDetalleLIQUIDO.AsFloat;
end;}

procedure TDMOfertas.InsertarOferta(id_s: integer; AgregarAlFinal, MantenerPrecioYDescuentos: boolean);
var
  FQ : TFIBQuery;
  id_detalles_s : integer;
  CantLineas : integer;
  Orden : integer;
  Linea, LineaAux : integer;
begin
  // Veo cuantas lineas tiene la oferta a insertar
  CantLineas := DMMain.DameLineaSiguiente(QMCabeceraTIPO.AsString, QMCabeceraID_S.AsInteger);

  id_detalles_s := QMDetalleID_DETALLES_S.AsInteger;
  // Linea a partir de la que inserto
  Orden := QMDetalleORDEN.AsInteger;

  QMDetalle.DisableControls;
  try
     if (not AgregarAlFinal) then
     begin
        // Avanzo el orden de las lineas
        with QMDetalle do
        begin
           // Renumero el resto de las lineas para generar hueco
           while (not EOF) do
           begin
              with THYFIBQuery.Create(nil) do
              begin
                 try
                    Close;
                    DataBase := DMMain.DataBase;
                    SQL.Text := 'UPDATE GES_DETALLES_S SET ORDEN = ORDEN + ?AVANCE WHERE ID_DETALLES_S = ?ID_DETALLES_S';
                    Params.ByName['AVANCE'].AsInteger := CantLineas - 1;
                    Params.ByName['ID_DETALLES_S'].AsInteger := QMDetalleID_DETALLES_S.AsInteger;
                    ExecQuery;
                    FreeHandle;
                 finally
     ...
     try
        ...
        begin
           ...
           try
              ...
              try
                 FSendCorreo.Titulo(_('Importando Oferta'));
                 FSendCorreo.Show;

                 if (not Transaction.InTransaction) then
                    Transaction.StartTransaction;
                 Close;
                 SQL.Text := 'SELECT ARTICULO, TITULO, UNIDADES, PRECIO, DESCUENTO, DESCUENTO_2, DESCUENTO_3 FROM GES_DETALLES_S WHERE ID_S = :ID_S ORDER BY ORDEN';
                 Params.ByName['ID_S'].AsInteger := id_s;
                 ExecQuery;
                 LineaAux := 1;
                 while (not EOF) do
                 begin
                    FSendCorreo.Texto(Format(_('Importando linea %d de %d'), [LineaAux, CantLineas]));
                    with QMDetalle do
                    begin
                       Append;
                       Linea := FieldByName('LINEA').AsInteger;
                       FieldByName('ARTICULO').AsString := FQ.FieldByName['ARTICULO'].AsString;
                       FieldByName('TITULO').AsString := FQ.FieldByName['TITULO'].AsString;
                       FieldByName('UNIDADES').AsFloat := FQ.FieldByName['UNIDADES'].AsFloat;
                       if (MantenerPrecioYDescuentos) then
                       begin
                          FieldByName('PRECIO').AsFloat := FQ.FieldByName['PRECIO'].AsFloat;
                          FieldByName('DESCUENTO').AsFloat := FQ.FieldByName['DESCUENTO'].AsFloat;
                          FieldByName('DESCUENTO_2').AsFloat := FQ.FieldByName['DESCUENTO_2'].AsFloat;
                          FieldByName('DESCUENTO_3').AsFloat := FQ.FieldByName['DESCUENTO_3'].AsFloat;
                       end;
                       Post;

                       if (not AgregarAlFinal) then
                       begin
                          // Asigno orden a la linea recien insertada
                          with THYFIBQuery.Create(nil) do
                          begin
                             try
                                Close;
                                DataBase := DMMain.DataBase;
                                SQL.Text := 'UPDATE GES_DETALLES_S SET ORDEN=?ORDEN WHERE ID_S = ?ID_S AND LINEA = ?LINEA';
                                Params.ByName['ORDEN'].AsInteger := Orden;
                                Params.ByName['ID_S'].AsInteger := QMCabeceraID_S.AsInteger;
  ...
  // Me posiciono sobre la ultima linea insertada
  QMCabecera.Refresh;
  Posicionar(QMDetalle, 'ID_DETALLES_S', id_detalles_s);
end;

function TDMOfertas.ExisteRango(Rango: integer): boolean;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMOfertas.QMCabeceraTIPO_PORTESChange(Sender: TField);
begin
...
procedure TDMOfertas.QMCabeceraINDICEChange(Sender: TField);
begin
  with xIndices do
  begin
     Close;
...
procedure TDMOfertas.QMCabeceraRANGOChange(Sender: TField);
begin
  with xRangos do
  begin
     Close;
...
procedure TDMOfertas.QMDetalleTIPO_UNIDAD_LOGISTICAChange(Sender: TField);
begin
  if ((QMDetalle.State = dsInsert) or (QMDetalle.State = dsEdit)) and (not FlagModificandoUnidades) then
  begin
     FlagModificandoUnidades := True;
...
procedure TDMOfertas.QMDetalleUNIDADES_LOGISTICASChange(Sender: TField);
var
  FactorUnidad : double;
begin
  if (Param_SYSCONF005 < 2) then
...
(*
procedure TDMOfertas.HabilitarBotones;
var
  Q : THYFIBQuery;
begin
  Q := THYFIBQuery.Create(nil);
  try
     ...
     begin
        ...
        Params.ByName['ID_DETALLES'].AsInteger := QMDetalleID_DETALLES_S.AsInteger;
        ExecQuery;
        if (FieldByName['ID_ESC'].AsInteger <> 0) then
        begin
           FMOfertas.TBOfertasEsc.ImageIndex := 7; // verd // escandallo
           FMOfertas.AProMedidas.Enabled := True;
           if (FieldByName['ID_MEDIDA'].AsInteger = 0) then // id_medida
              FMOfertas.AProMedidas.ImageIndex := 8 // groc
           else
              FMOfertas.AProMedidas.ImageIndex := 7; // verd
        end
        else
        begin
           FMOfertas.TBOfertasEsc.ImageIndex := 6; // vermell
           FMOfertas.AProMedidas.Enabled := False;
           FMOfertas.AProMedidas.ImageIndex := 6; // vermell
        end;
        FreeHandle;
     end;
  finally
...
procedure TDMOfertas.HabilitarBotonesEsc;
begin
  {
  if (LeeParametro('MODREST006') = 'S') then
  begin
     xBuscarEscandalloPed.Close;
     xBuscarEscandalloPed.Params.ByName['id_detalles'].AsInteger := QMDetalleID_DETALLES_S.AsInteger;
     xBuscarEscandalloPed.Prepare;
     xBuscarEscandalloPed.ExecQuery;

     if (xBuscarEscandalloPed.Fields[0].AsInteger <> 0) then
     begin
        FMPedidos.TBEscandall.ImageIndex := 7; //verd
        FMPedidos.AProCrearOF.Enabled := True;
        if (xBuscarEscandalloPed.Fields[2].AsInteger = 0) then //id_orden
           FMPedidos.AProCrearOF.ImageIndex := 8//groc
        else
           FMPedidos.AProCrearOF.ImageIndex := 7;//verd

        FMPedidos.AProADatosEsc.Enabled := True;
        if (xBuscarEscandalloPed.Fields[1].AsInteger = 0) then //id_medida
           FMPedidos.AProADatosEsc.ImageIndex := 8//groc
        else
           FMPedidos.AProADatosEsc.ImageIndex := 7;//verd
     end
     else
     begin
        FMPedidos.TBEscandall.ImageIndex := 6; //vermell
        FMPedidos.AProCrearOF.Enabled := False;
        FMPedidos.AProCrearOF.ImageIndex := 6;//vermell
        FMPedidos.AProADatosEsc.Enabled := False;
        FMPedidos.AProADatosEsc.ImageIndex := 6; //vermell
     end;

     xBuscarEscandalloPed.FreeHandle;
  end;
}
end;

procedure TDMOfertas.QMCabeceraDTO_PPChange(Sender: TField);
begin
...
procedure TDMOfertas.QMDetalleUNIDADESGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
...
// Andalplast
function TDMOfertas.BuscaOferta(id_detalles_s: integer): integer;
begin
  Result := 0;
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Text := 'SELECT RIG_OF FROM EMP_OFERTA WHERE ID_DETALLES_OFERTAS_V = ?ID_DETALLES_S';
           Params.ByName['ID_DETALLES_S'].AsInteger := id_detalles_s;
           ExecQuery;
           Result := FieldByName['RIG_OF'].AsInteger;
           FreeHandle;
        finally
           Free;
        end;
...
procedure TDMOfertas.QMCabeceraBeforeClose(DataSet: TDataSet);
begin
  xCliente.Close;
  xAlmacenes.Close;
  QMDetalle.Close;
...
procedure TDMOfertas.CrearOfertaCrm(IDAccion, IDContacto, Cliente, Agente: integer);
begin
  QMCabecera.Insert;
  ...
  if (Cliente <> 0) then
     ...
     QMCabeceraCLIENTE.AsInteger := -1;

  if (Agente > 0) then
     QMCabeceraAGENTE.AsInteger := Agente;
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Add(' UPDATE CRM_CONTACTOS_ACCIONES ');
           SQL.Add(' SET ');
           SQL.Add(' DOC_ACCION = :RIG, ');
           SQL.Add(' DOC_EMPRESAS = :EMPRESA, ');
           SQL.Add(' DOC_EJERCICIO = :EJERCICIO, ');
           SQL.Add(' DOC_CANAL = :CANAL, ');
           SQL.Add(' DOC_SERIES = :SERIE ');
           SQL.Add(' WHERE ');
           SQL.Add(' ID_ACCION = :ID_ACCION ');
           Params.ByName['RIG'].AsInteger := QMCabeceraRIG.AsInteger;
           Params.ByName['EMPRESA'].AsInteger := QMCabeceraEMPRESA.AsInteger;
           Params.ByName['EJERCICIO'].AsInteger := QMCabeceraEJERCICIO.AsInteger;
           Params.ByName['CANAL'].AsInteger := QMCabeceraCANAL.AsInteger;
  ...
  //S'inserta les dades del contacte provinents del contacto
  if ((QMCabeceraCLIENTE.AsInteger <= -1) and (IDContacto > 0)) then
  begin
     with THYFIBQuery.Create(nil) do
     begin
        try
           ...
           SQL.Add(' EMPRESA, EJERCICIO, CANAL, SERIE, TIPO, RIG, NIF, DIRECCION, C_POSTAL, PROVINCIA, NOMBRE, PAIS, TELEFONO01, TELEFONO02, ');
           SQL.Add(' TELEFAX, EMAIL, LOCALIDAD, TERCERO, PAIS_DOC_IDENT, TIPO_DOC_IDENT) ');
           SQL.Add(' SELECT :EMPRESA, :EJERCICIO, :CANAL, :SERIE, :TIPO, :RIG, C.NIF, SUBSTRING(C.DIR_NOMBRE FROM 1 FOR 100), L.CODPOSTAL, ');
           SQL.Add('        L.PROVINCIA, C.NOMBRE_COMERCIAL, C.PAIS, C.TELEFONO01, C.TELEFONO02, C.TELEFAX, C.EMAIL, L.TITULO, C.TERCERO, ');
           SQL.Add('        L.C_PAIS, ');
           SQL.Add('        (SELECT TIPO_DOC_IDENT ');
           SQL.Add('         FROM SYS_TIPO_DOC_IDENT_PAIS ');
           SQL.Add('         WHERE ');
           SQL.Add('         PAIS = L.C_PAIS AND ');
           SQL.Add('         DEFECTO = 1) ');
           SQL.Add(' FROM CRM_CONTACTOS C ');
           SQL.Add(' LEFT JOIN VER_LOCALIDADES L ON L.LOCALIDAD = C.DIR_LOCALIDAD ');
           SQL.Add(' WHERE ');
           SQL.Add(' C.ID_CONTACTO = :ID_CONTACTO ');
           Params.ByName['EMPRESA'].AsInteger := REntorno.Empresa;
           Params.ByName['EJERCICIO'].AsInteger := REntorno.Ejercicio;
           Params.ByName['CANAL'].AsInteger := REntorno.Canal;
           Params.ByName['SERIE'].AsString := QMCabeceraSERIE.AsString;
...
procedure TDMOfertas.CrearOfertaHubspot(Cliente: integer);
begin
  QMCabecera.Insert;
...
procedure TDMOfertas.BusquedaArticulo(Descripcion: string);
var
...
begin
  ...
  try
     ...
     QMDetalleARTICULO.AsString := DMMain.BusquedaArticulo(Descripcion, QMCabeceraALMACEN.AsString, QMCabeceraCLIENTE.AsInteger);
  finally
     QMDetalleARTICULO.OnChange := evt;
  end;
end;

procedure TDMOfertas.RellenaDatosOldLinea;
begin
  RellenaDatosLinea(OldLinea);
end;

procedure TDMOfertas.RellenaDatosLinea(Linea: TStrings);
...
procedure TDMOfertas.AsignarImagenLinea;
begin
  // Si existe la linea (no es una line vacia)
  if (QMDetalleRIG.AsInteger <> 0) then
  begin
     // Me aseguro de que esté en edicion
     if not (QMDetalle.State in [dsInsert, dsEdit]) then
        QMDetalle.Edit;

     QMDetalleID_IMAGEN.AsInteger := ImportarImagen('');
  end;
end;

procedure TDMOfertas.xInfoActualizadaTITULO_ESTADOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := DameTituloEstado(xInfoActualizadaESTADO.AsInteger);
end;

procedure TDMOfertas.DuplicarLinea;
var
  old_KriConf462 : boolean;
  IdDetallesS, NuevaLinea : integer;
  Precio : double;
begin
  old_KriConf462 := KriConf462;
  try
     KriConf462 := True;
     IdDetallesS := QMDetalleID_DETALLES_S.AsInteger;

     // Guardo los datos con lo que hay en la linea si KriConf462 estaba deshabilidatdo
     ...
     KriConf462 := old_KriConf462;
  end;
end;

procedure TDMOfertas.Despiece;
{
var
  IdDespiece : integer;
}
begin
  DMMain.LogIni('DMOfertas.Despiece');
  ...
  if (QMDetalleID_DESPIECE.AsInteger = 0) then
     FMDespiece.SegunProcedencia('OFC', QMDetalleID_DETALLES_S.AsInteger, QMCabeceraID_S.AsInteger, QMDetalle)
  else
  ...
  begin
     ...
     IdDespiece := FMDespiece.SegunProcedencia('OFC', QMDetalleID_DETALLES_S.AsInteger, QMCabeceraID_S.AsInteger, QMDetalle);
...
function TDMOfertas.GeneraReparacion(Serie: string; Fecha: TDateTime): integer;
begin
  // Genera Documento de Reparacion desde oferta
  ...
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'EXECUTE PROCEDURE G_TRASPASO_DOC_A_REPARACION(:ID_S, :SERIE_DST, :FECHA_DST)';
        Params.ByName['ID_S'].AsInteger := QMCabeceraID_S.AsInteger;
        Params.ByName['SERIE_DST'].AsString := Serie;
        Params.ByName['FECHA_DST'].AsDateTime := Fecha;
        ExecQuery;
        Result := FieldByName['IDCABREPARAR'].AsInteger;
        FreeHandle;
     finally
        Free;
     end;
...
function TDMOfertas.DameIDREparacionOferta(id_s: integer): integer;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'SELECT IDCABREPARAR FROM REPAR_CAB_REPARACIONES WHERE ID_S_ORIGEN = ?ID_S';
        Params.ByName['ID_S'].AsInteger := id_s;
        ExecQuery;
        Result := FieldByName['IDCABREPARAR'].AsInteger;
        FreeHandle;
...
procedure TDMOfertas.QMDetalleCalcFields(DataSet: TDataSet);
var
  i : integer;
  Stock : double;
  Articulo : string;
...
procedure TDMOfertas.AsignaEmpleados;
begin
  UFMAsignaEmpleado.PideDato(QMCabeceraID_S.AsInteger);
end;

procedure TDMOfertas.QMCabeceraCalcFields(DataSet: TDataSet);
begin
...
procedure TDMOfertas.CreaReciboAnticipo;
var
  IdCartera : integer;
...
procedure TDMOfertas.ActualizaPorteSegunZona;
var
  Zona : string;
  CodigoPostal : string;
...
procedure TDMOfertas.QMCabeceraTRANSPORTISTAChange(Sender: TField);
begin
  if (xCliente.FieldByName('TIPO_PORTES').AsInteger = 0) then
     ActualizaPorteSegunZona;
end;

procedure TDMOfertas.FiltraDocumento(Serie, Filtro: string);
begin
  with QMCabecera do
  begin
     Close;
...
procedure TDMOfertas.PosicionarID(IdDoc: integer);
begin
  // Si pertenece al ejercicio posiciono, sino filtro ese documento
  if (DMMain.DameEjercicioDoc('OFC', IdDoc) = REntorno.Ejercicio) then
  begin
...
procedure TDMOfertas.QMCabeceraVALOR_CAMB_FIJOChange(Sender: TField);
begin
...
procedure TDMOfertas.QMCabeceraVALOR_CAMB_FIJO_INVChange(Sender: TField);
begin
  if (not ModificandoCambioFijo) then
  begin
     ModificandoCambioFijo := True;
...
procedure TDMOfertas.Inserta(Articulo: string; Unidades: double; Lote: string; IdUbicacion: integer; Precio: double; Descripcion: string; PCoste, Descuento1, Descuento2, Descuento3: double; FechaPrevista: TDateTime);
...
procedure TDMOfertas.ReiniciaStock(Articulo: string = '');
var
  i : integer;
begin
...
procedure TDMOfertas.QMDetallePESOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetallePESO.AsFloat]);
end;

procedure TDMOfertas.QMDetallePESO_TOTALGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetallePESO_TOTAL.AsFloat]);
end;

procedure TDMOfertas.QMDetallePESO_REALGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetallePESO_REAL.AsFloat]);
end;

procedure TDMOfertas.QMDetalleTIPO_IVAChange(Sender: TField);
begin
  QMDetalleP_IVA.AsFloat := DMMain.DamePorcentajeIva(REntorno.Pais, QMDetalleTIPO_IVA.AsInteger);
end;

procedure TDMOfertas.QMDetallePESO_REALChange(Sender: TField);
begin
  FlagModificandoUnidades := True;
  try
     if Param_VENULKG001 and (QMDetalleTIPO_UNIDAD_LOGISTICA.AsString = 'KG') then
...
procedure TDMOfertas.QMDetalleALMACENChange(Sender: TField);
begin
  if (param_VENPREC002) then
     EstaleceCondiciones(QMDetalleARTICULO, 0, (QMDetalle.State in [dsInsert, dsEdit]));
end;

procedure TDMOfertas.xInfoActualizadaCalcFields(DataSet: TDataSet);
begin
...
procedure TDMOfertas.QMDetalleUNIDADES_EXTGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetalleUNIDADES_EXT.AsFloat]);
end;

procedure TDMOfertas.QMDetalleTOTAL_UNIDADES_EXTGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
  Text := Format('%.*f', [QMDetalleDECIMALES_UNIDADES.AsInteger, QMDetalleTOTAL_UNIDADES_EXT.AsFloat]);
end;

procedure TDMOfertas.ImportaDetalleDesdeExcel(Archivo: string);
var
  HCalc : THojaCalc;
  i, IdA, IdDetallesS, Dir_Distribucion, DireccionFinal, Bultos, CantidadEmpaque : integer;
  s, Articulo, Sku, {Lote,} NroOrden, TituloProducto : string;
  Cancelar : boolean;
  Unidades{, Precio} : double;
  FechaDocumento, FechaEntrega : TDateTime;
        ...
        begin
           ...
           begin
              ...
              begin
                 ...
                 NroOrden := Trim(s);
                 if (NroOrden = '') then
                    Cancelar := (MessageDlg(Format(_('No existe Nro de orden' + #13#10 + 'Linea: %d.' + #13#10 + '¿Desea continuar?'), [i]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              ...
              begin
                 ...
                 if (Dir_Distribucion = 0) then
                    Cancelar := (MessageDlg(Format(_('No se encontro la direccion de distribuicion para el cliente.' + #13#10 + 'Leido Excel: %s.' + #13#10 + '¿Desea continuar?'), [s]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              end;
              ...
              begin
                 ...
                 if (Articulo = '') then
                 begin
                    Cancelar := (MessageDlg(Format(_('No existe Ariculo para el SKU leido' + #13#10 + 'Leido Excel: %s.' + #13#10 + '¿Desea continuar?'), [Sku]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              ...
              begin
                 ...
                 if (IdA = 0) then
                    Cancelar := (MessageDlg(Format(_('No se encontro ID_A de Ariculo' + #13#10 + 'Código: %s.' + #13#10 + '¿Desea continuar?'), [Articulo]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              end;
              ...
              begin
                 ...
                 if (TituloProducto = '') then
                    Cancelar := (MessageDlg(Format(_('No existe titulo del producto' + #13#10 + 'Leido Excel: %s.' + #13#10 + '¿Desea continuar?'), [s]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              end;
              ...
              begin
                 ...
                 if (DireccionFinal = 0) then
                    Cancelar := (MessageDlg(Format(_('No se encontro la direccion final de entrega para el cliente' + #13#10 + 'Leido Excel: %s.' + #13#10 + '¿Desea continuar?'), [s]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              end;
              ...
              begin
                 ...
                 if (Unidades = 0) then
                    Cancelar := (MessageDlg(Format(_('Las unidades son 0.' + #13#10 + 'Leido Excel: %s.' + #13#10 + '¿Desea continuar?'), [s]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              end;
              ...
              begin
                 ...
                 if (CantidadEmpaque = 0) then
                    Cancelar := (MessageDlg(Format(_('Las cantidad empaque es 0.' + #13#10 + 'Leido Excel: %s.' + #13#10 + '¿Desea continuar?'), [s]), mtWarning, [mbYes, mbNo], 0) = mrNo);
              end;
              ...
              begin
                 ...
                 begin
                    try
                       Close;
                       DataBase := DMMain.DataBase;
                       SQL.Text := 'SELECT ID_DETALLES_S FROM GES_DETALLES_S WHERE ID_S = :ID_S AND ARTICULO = :ARTICULO ORDER BY ORDEN';
                       Params.ByName['ID_S'].AsInteger := QMCabeceraID_S.AsInteger;
                       Params.ByName['ARTICULO'].AsString := Articulo;
                       ExecQuery;
                 ...
                 begin
                    ...
                    begin
                       ...
                       FieldByName('TITULO').AsString := DameTituloArticulo(IdA);
...
procedure TDMOfertas.InsertaArticulosCliente;
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'EXECUTE PROCEDURE RECUPERA_LINEAS_FAC_EJERCICIO (:ID_S)';
        Params.ByName['ID_S'].AsInteger := QMCabeceraID_S.AsInteger;
        ExecQuery;
        FreeHandle;
     finally
...
procedure TDMOfertas.xClienteAfterOpen(DataSet: TDataSet);
begin
  xAvisos.Open;
end;

procedure TDMOfertas.xClienteBeforeOpen(DataSet: TDataSet);
begin
  xAvisos.Close;
end;

procedure TDMOfertas.RecuperarCostesEscandallo;
begin
  /// Crea lineas de detalle con las diferentes unidades y costes y precios de venta del escandallo.
  ...
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'EXECUTE PROCEDURE TRASPASO_COSTES_A_OFERTA(:ID_DETALLES_S)';
        Params.ByName['ID_DETALLES_S'].AsInteger := QMDetalleID_DETALLES_S.AsInteger;
        ExecQuery;
        FreeHandle;
     finally
...
procedure TDMOfertas.xDireccionesTITULOGetText(Sender: TField; var Text: string; DisplayText: boolean);
begin
...
procedure TDMOfertas.QMDetalleUNIDADES_SECChange(Sender: TField);
begin
  // Si calcula el peso segun las unidades secundarias
  if param_VENUSEC001 and (QMDetalleAPLICA_UNIDADES_SECUNDARIAS.AsInteger = 1) then
...
procedure TDMOfertas.QMDetalleBeforeInsert(DataSet: TDataSet);
begin
  DMMain.RestingeEdicion(DataSet, QMCabeceraESTADO.AsInteger);
end;

procedure TDMOfertas.ConectaProveedores;
begin
  xProveedores.Open;
end;

procedure TDMOfertas.DesconectaProveedores;
begin
  xProveedores.Close;
end;

procedure TDMOfertas.xProveedoresNewRecord(DataSet: TDataSet);
begin
...
procedure TDMOfertas.xProveedoresARTICULOChange(Sender: TField);
begin
...
procedure TDMOfertas.xProveedoresAfterPost(DataSet: TDataSet);
begin
...
procedure TDMOfertas.CambiaPCosteLinea(PCoste: double);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMOfertas.CambiaCosteAdicionalLinea(CosteAdicional: double);
begin
...
procedure TDMOfertas.QMCabeceraRECHAZADAChange(Sender: TField);
begin
  if (QMCabeceraRECHAZADA.AsInteger = 1) then
     QMCabeceraRECHAZADA_FECHA.AsDateTime := Today;
end;

procedure TDMOfertas.MultiplicaPrecios(Factor: double);
var
  id : integer;
begin
  DMMain.LogIni('DMOfertas.MultiplicaPrecios');
