...
type
  TDMAsistenteImpModelos = class(TDataModule)
     TLocal: THYTransaction;
     TUpdate: THYTransaction;
     QImportaModelo: THYFIBQuery;
...
var
  DMAsistenteImpModelos : TDMAsistenteImpModelos;

implementation

uses UDMMain, UEntorno, Variants, UUtiles, Dialogs, StrUtils, UHojaCalc, UFMAsistenteImpModelos, FIBDataSetRW;

{$R *.dfm}

procedure TDMAsistenteImpModelos.DataModuleCreate(Sender: TObject);
begin
...
procedure TDMAsistenteImpModelos.DataModuleDestroy(Sender: TObject);
begin
  Filas.Free;
end;

function TDMAsistenteImpModelos.LeerDatos(Archivo, Hoja: string): boolean;
var
  HCalc : THojaCalc;
  f, c, x, y : integer;
  ...
  try
     ...
     try
        ...
        try
           ...
           FMAsistenteImpModelos.ProgresoImportacion.Min := 0;
           FMAsistenteImpModelos.ProgresoImportacion.Max := y;
           ...
           begin
              ...
              begin
                 try
                    ...
                    on e: Exception do
                    begin
                       ShowMessage(_('ERROR ' + #13#10 + e.Message));
                       StrList.Add(_('*'));
                    end;
                 end;
              end;
              ...
              FMAsistenteImpModelos.ProgresoImportacion.Position := c;
           end;
...
function TDMAsistenteImpModelos.DameFilas: TObjectList;
begin
  Result := Filas;
end;

function TDMAsistenteImpModelos.DameColumnas: TStrings;
begin
  Result := (Filas.Items[0] as TStrings);
end;

procedure TDMAsistenteImpModelos.RellenaCampos(Campos: TStrings);
var
...
begin
  ...
  try
     ...
     begin
        ...
        SelectSQL.Text := 'SELECT * FROM VER_MODELOS_IMPORTACION WHERE EMPRESA = 0';
        Open;
...
procedure TDMAsistenteImpModelos.InicializaImportacion(aSeparadorDecimal: char);
var
...
begin
  ...
  try
     DMAsistenteImpModelos.RellenaCampos(ListaCampos);
  ...
  with QImportaModelo do
  begin
     SQL.Clear;
     SQL.Add(' INSERT INTO VER_MODELOS_IMPORTACION');
     SQL.Add(' (' + Campos + ')');
     SQL.Add(' VALUES ');
     SQL.Add(' (' + Valores + ')');
  end;

  DMMain.Log(QImportaModelo.SQL.Text);
end;

procedure TDMAsistenteImpModelos.NuevoArticulo;
var
  i : integer;
begin
  DMMain.Log('NuevoModelo');
  with QImportaModelo do
  begin
...
procedure TDMAsistenteImpModelos.AsignaParametro(Parametro, Valor: string);
var
  i, j : integer;
  s : string;
begin
     ...
     for i := 0 to Params.Count - 1 do
        ...
        begin
           ...
           begin
              ...
              begin
                 FMAsistenteImpModelos.Log(Format(_('Se reduce el tamaño del campo %s. (Max. = %d). Se importa: %s'), [Params[i].Name, Params[i].Size, Copy(Valor, 1, Params[i].Size)]));
                 FMAsistenteImpModelos.Log('');
              end;
...
procedure TDMAsistenteImpModelos.CrearModelo;
var
  i : integer;
begin
  // Crea el Articulo, rellenando algunos parametros si fuera necesario.
  DMMain.Log('CrearModelo');
  with QImportaModelo do
  begin
     for i := 0 to Params.Count - 1 do
     begin
...
procedure TDMAsistenteImpModelos.ActualizaDatosImportacion(aFileName: string; var aLastRow, aLastCol, aCountSheet: integer; var aActiveSheetName: string; Sheets: TStrings);
var
  i : integer;
begin
  aLastRow := 0;
