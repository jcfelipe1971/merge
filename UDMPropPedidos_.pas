...
type
  ...
  public
     ...
     procedure DesmarcarExcepto(Almacen, Proveedor: string; Modo: integer; TipoArticulo, DesdeArticulo, HastaArticulo, DesdeArticulo2, HastaArticulo2, DesdeFamilia, HastaFamilia: string);
  end;
...
uses UDMMain, UEntorno, UParam, UUtiles, UFBusca, DateUtils, UFMSeleccion;
...
begin
  ...
  QMPedidosEXISTENCIAS.DisplayFormat := MascaraN;
  QMPedidosMINIMO.DisplayFormat := MascaraN;
  QMPedidosMAXIMO.DisplayFormat := MascaraN;
  QMPedidosP_MINIMO.DisplayFormat := MascaraN;
  QMPedidosP_OPTIMO.DisplayFormat := MascaraN;
  QMPedidosPEDIDOS_D_CLI.DisplayFormat := MascaraN;
  QMPedidosPEDIDOS_A_PRO.DisplayFormat := MascaraN;
  QMPedidosPEDIR.DisplayFormat := MascaraN;
  QMPedidosUNIDADES_PEDIDO.DisplayFormat := MascaraN;
  QMPedidosUNIDADES_PRODUCCION.DisplayFormat := MascaraN;
  QMPedidosSTOCK_VIRTUAL.DisplayFormat := MascaraN;
  QMPedidosEN_PRODUCCION.DisplayFormat := MascaraN;
  QMPedidosVENTAS_PERIODO_ANTERIOR.DisplayFormat := MascaraN;
  QMPedidosCOBERTURA.DisplayFormat := MascaraN;
  QMPedidosROTURA.DisplayFormat := MascaraN;
  QMPedidosUNIDADES_COBERTURA.DisplayFormat := MascaraN;
  QMPedidosCOBERTURA_ROTURA.DisplayFormat := MascaraN;

  QMPedidosTotalesUNIDADES_PEDIDAS.DisplayFormat := MascaraN;
...
procedure TDMPropPedidos.DesmarcarExcepto(Almacen, Proveedor: string; Modo: integer; TipoArticulo, DesdeArticulo, HastaArticulo, DesdeArticulo2, HastaArticulo2, DesdeFamilia, HastaFamilia: string);
begin
  /// Descarca los registro excepto los que coniciden con el filtro.
  ...
  begin
     try
        ...
        SQL.Add(' MARCADO = 1 ');
        if (Almacen <> '') then
           SQL.Add(' AND ALMACEN = ''' + Copy(Almacen, 1, 3) + ''' ');
        if (Proveedor <> '') then
           SQL.Add(' AND PROVEEDOR = ' + Proveedor);

        if (TipoArticulo <> '') then
           SQL.Add(' AND TIPO_ARTICULO = ''' + Trim(TipoArticulo) + ''' ');
        if (DesdeArticulo2 <> '') then
        begin
           SQL.Add(' AND (( ');
           if (DesdeArticulo <> '') then
              SQL.Add(' ARTICULO >= ''' + Trim(DesdeArticulo) + ''' ');
           if (HastaArticulo <> '') then
              SQL.Add(' AND ARTICULO <= ''' + Trim(HastaArticulo) + ''' ');
           SQL.Add(' ) OR ( ');
           if (DesdeArticulo2 <> '') then
              SQL.Add(' ARTICULO >= ''' + Trim(DesdeArticulo2) + ''' ');
           if (HastaArticulo2 <> '') then
              SQL.Add(' AND ARTICULO <= ''' + Trim(HastaArticulo2) + ''' ');
           SQL.Add(' )) ');
        end
        else
        begin
           if (DesdeArticulo <> '') then
              SQL.Add(' AND ARTICULO >= ''' + Trim(DesdeArticulo) + ''' ');
           if (HastaArticulo <> '') then
              SQL.Add(' AND ARTICULO <= ''' + Trim(HastaArticulo) + ''' ');
        end;
        if (DesdeFamilia <> '') then
           SQL.Add(' AND FAMILIA >= ''' + Trim(DesdeFamilia) + ''' ');
        if (HastaFamilia <> '') then
           SQL.Add(' AND FAMILIA <= ''' + Trim(HastaFamilia) + ''' ');
        SQL.Add(' ) ');
...
begin
  QMPedidosNOMBRE_R_SOCIAL.AsString := DMMain.DameTituloProveedor(StrToIntDef(QMPedidosPROVEEDOR.AsString, 0));
end;
