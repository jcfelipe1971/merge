...
type
  TFMAsistenteImpModelos = class(TFPEditSinNavegador)
  ...
  private
     ...
     procedure ImportarModelos(BorraLog: boolean = True);
  public
     { Public declarations }
...
var
  FMAsistenteImpModelos : TFMAsistenteImpModelos;

implementation

uses UDMMain, UEntorno, UDMAsistenteImpModelos, Contnrs, UUtiles;

{$R *.dfm}

procedure TFMAsistenteImpModelos.FormCreate(Sender: TObject);
var
  i : integer;
  Campos : TStrings;
  MI : TMenuItem;
begin
  ...
  AbreData(TDMAsistenteImpModelos, DMAsistenteImpModelos);
  InicializaVariables;

  EdArchivoImp.Text := LeeDatoIni('ImportacionModelos', 'UltimoArchivo', '');
  CBHojaAImportar.Text := LeeDatoIni('ImportacionModelos', 'UltimaHoja', '');
  ...
  try
     DMAsistenteImpModelos.RellenaCampos(Campos);
...
procedure TFMAsistenteImpModelos.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  CierraData(DMAsistenteImpModelos);
  Action := caFree;
end;

{ PROCEDIMIENTOS DEL MOTOR DEL ASISTENTE }

procedure TFMAsistenteImpModelos.InicializaVariables;
begin
  // Expandir nodo
...
procedure TFMAsistenteImpModelos.ActivarTS(Index: integer);
var
  i : integer;
begin
...
procedure TFMAsistenteImpModelos.ButtAntClick(Sender: TObject);
begin
  MueveTab(-1);
end;

procedure TFMAsistenteImpModelos.ButtNextClick(Sender: TObject);
begin
  MueveTab(1);
end;

procedure TFMAsistenteImpModelos.MueveTab(Valor: integer);
begin
  Cambiar := False;
...
procedure TFMAsistenteImpModelos.ActivarTree(Index: integer);
begin
  TVMain.Items[0].Item[index].Selected := True;
end;

procedure TFMAsistenteImpModelos.TVMainCollapsed(Sender: TObject; Node: TTreeNode);
begin
  TVMain.Items[0].Expanded := True;
end;

procedure TFMAsistenteImpModelos.TVMainChange(Sender: TObject; Node: TTreeNode);
begin
  MueveTabTree(Node.Index);
end;

procedure TFMAsistenteImpModelos.ControlaBotones(Index: integer);
begin
  if (TabIndice = 0) then
...
procedure TFMAsistenteImpModelos.MueveTabTree(Index: integer);
begin
  if not (Cambiar) then
  begin
...
function TFMAsistenteImpModelos.CheckTab(Index: integer): boolean;
begin
...
procedure TFMAsistenteImpModelos.ButtCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TFMAsistenteImpModelos.ButtSelArchivoClick(Sender: TObject);
var
...
begin
  ...
  begin
     ...
     try
        ...
        DMAsistenteImpModelos.ActualizaDatosImportacion(Archivo, Filas, Columnas, CantidadHojas, HojaActiva, Sheets);
...
procedure TFMAsistenteImpModelos.RefrescaInformacionArchivo;
var
  Filas, Columnas, CantidadHojas : integer;
  HojaActiva : string;
  Sheets : TStrings;
begin
  ...
  try
     HojaActiva := CBHojaAImportar.Text;
     DMAsistenteImpModelos.ActualizaDatosImportacion(EdArchivoImp.Text, Filas, Columnas, CantidadHojas, HojaActiva, Sheets);
...
procedure TFMAsistenteImpModelos.TSSeleccionArchivoShow(Sender: TObject);
begin
  EdArchivoImp.SetFocus;
end;

function TFMAsistenteImpModelos.CheckArchivoImp: boolean;
begin
  Result := False;
  ...
  Result := DMAsistenteImpModelos.LeerDatos(EdArchivoImp.Text, CBHojaAImportar.Text);
  if (Result) then
  begin
     LlenarGrid;
     LlenarConstantes;

     EscribeDatoIni('ImportacionModelos', 'UltimoArchivo', Trim(EdArchivoImp.Text));
     EscribeDatoIni('ImportacionModelos', 'UltimaHoja', Trim(CBHojaAImportar.Text));
     EscribeDatoIni(Trim(EdArchivoImp.Text) + '||' + Trim(CBHojaAImportar.Text), 'FechaImportacion', Now);

     LFCBPrimeraFilaTitulos.Checked := LeeDatoIni('ImportacionModelos', 'PrimeraFilaTitulos', False);
     ESeparadorDecimal.Text := LeeDatoIni('ImportacionModelos', 'SeparadorDecimal', '');
  end;
end;

procedure TFMAsistenteImpModelos.LlenarGrid;
var
  ColCount, x, f, c : integer;
  Filas : TObjectList;
begin
  Filas := DMAsistenteImpModelos.DameFilas;
...
procedure TFMAsistenteImpModelos.LlenarConstantes;
var
  i : integer;
  Campos : TStrings;
begin
...
procedure TFMAsistenteImpModelos.SGDatosImpDrawCell(Sender: TObject; ACol, ARow: integer; Rect: TRect; State: TGridDrawState);
begin
...
procedure TFMAsistenteImpModelos.LimpiarLog;
begin
  MEMInfoProc.Lines.Clear;
end;

procedure TFMAsistenteImpModelos.Log(Texto: string; NuevaLinea: boolean = True);
begin
  if NuevaLinea then
...
procedure TFMAsistenteImpModelos.ImportarModelos(BorraLog: boolean = True);
var
...
begin
  ...
  DMAsistenteImpModelos.InicializaImportacion(ESeparadorDecimal.Text[1]);
  Seguir := True;
  f := PrimeraFila;
  ...
  begin
     DMAsistenteImpModelos.NuevoArticulo;
     ...
     begin
        ...
        begin
           DMAsistenteImpModelos.AsignaParametro(SGDatosImp.Cells[c, 0], SGDatosImp.Cells[c, f]);
     ...
     begin
        ...
        begin
           DMAsistenteImpModelos.AsignaParametro(VLEValoresConstantes.Keys[c], VLEValoresConstantes.Values[VLEValoresConstantes.Keys[c]]);
           // Log(format(_('Constantes: "%s" -> "%s"'), [VLEValoresConstantes.Keys[c], VLEValoresConstantes.Values[VLEValoresConstantes.Keys[c]]]));
        end;
     end;

     try
        DMAsistenteImpModelos.CrearModelo;
     except
        on e: Exception do
        begin
           Seguir := False;
           Log(_('Error al importar Modelo'));
           Log('----------');
           Log(E.Message);
           Log('----------');
           for c := 0 to SGDatosImp.ColCount - 1 do
...
procedure TFMAsistenteImpModelos.ButtCrearClick(Sender: TObject);
begin
  ImportarModelos;
end;

procedure TFMAsistenteImpModelos.ButtCerrarClick(Sender: TObject);
begin
  Close;
end;

procedure TFMAsistenteImpModelos.PMCamposDisponiblesClick(Sender: TObject);
begin
...
procedure TFMAsistenteImpModelos.LFCBPrimeraFilaTitulosChange(Sender: TObject);
begin
  if LFCBPrimeraFilaTitulos.Checked then
  ...
  EscribeDatoIni('ImportacionModelos', 'PrimeraFilaTitulos', LFCBPrimeraFilaTitulos.Checked);
end;

procedure TFMAsistenteImpModelos.VLEValoresConstantesExit(Sender: TObject);
var
  i : integer;
begin
  // Primera linea contiene los titulos ("Key" - "Value")
...
procedure TFMAsistenteImpModelos.EdArchivoImpChange(Sender: TObject);
begin
  inherited;
  RefrescaInformacionArchivo;
end;

procedure TFMAsistenteImpModelos.CBHojaAImportarChange(Sender: TObject);
begin
  inherited;
  RefrescaInformacionArchivo;
end;

procedure TFMAsistenteImpModelos.TSImportarShow(Sender: TObject);
begin
  inherited;
  EUltimaFilaImportada.Text := IntToStr(LeeDatoIni(Trim(EdArchivoImp.Text) + '||' + Trim(CBHojaAImportar.Text), 'UltimaFilaImportada', 0));
end;

procedure TFMAsistenteImpModelos.BContinuarClick(Sender: TObject);
begin
  inherited;
  ...
  ImportarModelos(False);
end;

procedure TFMAsistenteImpModelos.ESeparadorDecimalChange(Sender: TObject);
begin
  inherited;
  EscribeDatoIni('ImportacionModelos', 'SeparadorDecimal', ESeparadorDecimal.Text);
end;
