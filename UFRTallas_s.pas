...
uses {IDIOMA_CODE}gnugettext{IDIOMA_CODE},
  ...
  UHYDBGrid, ExtCtrls, ToolWin, ComCtrls, DB, UDMTallas_s, Variants,
...
type
  TFRTallas_s = class(TFrame)
     PNLDetalleTallas: TLFPanel;
     DBGFDetalleTallas: TDBGridFind2000;
     TBMain: TLFToolBar;
     DSQMver_detalles_gr_sd: TDataSource;
     ...
     DSQMver_detalles_gr_sd_pv: TDataSource;
     ATraspasaLineaOrdenProduccion: TAction;
     TButtTraspasaLineaAOrdenProduccion: TToolButton;
     TBTraspasaPedidoAOrdenProduccion: TToolButton;
     ATraspasaPedidoAOrdenProduccion: TAction;
     TBTraspasaAgrupPedidoAOrdenProduccion: TToolButton;
     ATraspasaAgrupPedidoAOrdenProduccion: TAction;
     ANuevoArticulo: TAction;
     TBNuevoArticulo: TToolButton;
     ...
     procedure ATraspasaLineaOrdenProduccionExecute(Sender: TObject);
     procedure ATraspasaPedidoAOrdenProduccionExecute(Sender: TObject);
     procedure ATraspasaAgrupPedidoAOrdenProduccionExecute(Sender: TObject);
     procedure ANuevoArticuloExecute(Sender: TObject);
  ...
  public
     ...
     procedure SetDM(dm: TDMTallas_s);
...
uses UDMMain, UFMStockTallas, UDMTallas, UFMain, UEntorno, UFMSeleccion, UUtiles,
  UFMTraspasoPedidosAProduccionTyCKri, UFMModelosTallas, UFormGest, HYFIBQuery;

{$R *.dfm}

procedure TFRTallas_s.DBGFDetalleTallasRowChange(Sender: TObject);
begin
  AjustarGrid;
end;

procedure TFRTallas_s.SetDM(dm: TDMTallas_s);
begin
  Self.dm := dm;
  DSQMver_detalles_gr_sd.DataSet := dm.QMver_detalles_gr_sd;
  DSQMver_detalles_gr_sd_pv.DataSet := dm.QMver_detalles_gr_sd_pv;
end;

procedure TFRTallas_s.DBGFDetalleTallasBusqueda(Sender: TObject);
begin
  ...
  begin
     ...
     begin
        ...
        CondicionBusqueda := 'EMPRESA=' + REntorno.EmpresaStr + ' AND REF_PROVEEDOR =''' +
           VartoStr(dm.QMver_detalles_gr_sd['REF_PROVEEDOR']) + '''';
     end;
  end;
end;

procedure TFRTallas_s.DBGFDetalleTallasExiste(Sender: TObject);
begin
  AjustarGrid;
end;

procedure TFRTallas_s.DBGFDetalleTallasCampoDevuelve(Sender: TObject; valor: string);
begin
  AjustarGrid;
end;

procedure TFRTallas_s.AStockTallasExecute(Sender: TObject);
var
  ID_A_M_C : variant;
begin
  ID_A_M_C := null;
  if dm.QMver_detalles_gr_sd['ID_A_M_C'] <> null then
     ID_A_M_C := dm.QMver_detalles_gr_sd['ID_A_M_C'];
  if dm.QMver_detalles_gr_sd_pv['ID_A_M_C'] <> null then
     ID_A_M_C := dm.QMver_detalles_gr_sd_pv['ID_A_M_C'];
  if ID_A_M_C <> null then
     TFMStockTallas.Create(Self).MuestraAMC(ID_A_M_C);
end;

procedure TFRTallas_s.TCPreciosChange(Sender: TObject);
begin
  case TCPrecios.TabIndex of
     0: // Unidades
     begin
        dm.QMver_detalles_gr_sd.Open;
        DBGFDetalleTallas.Visible := True;
        DBGFDetalleTallas.SetFocus;
        DBGFDetalleTallasPrecios.Visible := False;
        NavTallas.DataSource := DSQMver_detalles_gr_sd;
        ...
        dm.QMver_detalles_gr_sd_pv.Close;
     end;
     1: // Precios
     begin
        dm.QMver_detalles_gr_sd_pv.Open;
        ...
        NavTallas.DataSource := DSQMver_detalles_gr_sd_pv;
        DMTallas.SetCaptions(dm.QMver_detalles_gr_sd_pv['ID_G_T'],
           DBGFDetalleTallasPrecios);
        dm.SyncTallas(TCPrecios.TabIndex);
        with DBGFDetalleTallasPrecios do
           SelectedField := DataSource.DataSet.FindField(
              'P' + Copy(DBGFDetalleTallas.SelectedField.FieldName, 2, 2));
        dm.QMver_detalles_gr_sd.Close;
     end;
  end;
  AjustarGrid;
end;

procedure TFRTallas_s.DBGFDetalleTallasPreciosRowChange(Sender: TObject);
begin
  AjustarGrid;
end;

procedure TFRTallas_s.AjustarGrid;
var
  ID_G_T, ID_A_M_C : variant;
begin
  ID_G_T := dm.GetActive(TCPrecios.TabIndex)['ID_G_T'];
...
procedure TFRTallas_s.DBGFDetalleTallasDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: integer; Column: TColumn; State: TGridDrawState);
begin
  with TDBGridFind2000(Sender) do
  begin
     if (Column.Field.FieldName = 'P_VENTA') and
        (dm.QMver_detalles_gr_sd['CAMBIA'] <> null) and
        (dm.QMver_detalles_gr_sd['CAMBIA'] <> 0) then
     ...
     begin
        if ((dm.QMver_detalles_gr_sd[Column.Field.FieldName] <> null)) then
           ColorEdicion(Canvas)
        else
...
procedure TFRTallas_s.LocateTalla(cod_talla: string);
begin
     ...
     case TCPrecios.TabIndex of
        ...
        begin
           ...
           DBGFDetalleTallas.SelectedField :=
              dm.QMver_detalles_gr_sd.FindField('T' + cod_talla);
        ...
        begin
           ...
           DBGFDetalleTallasPrecios.SelectedField :=
              dm.QMver_detalles_gr_sd_pv.FindField('P' + cod_talla);
        end;
     end;
  end;
end;

procedure TFRTallas_s.DBGFDetalleTallasColEnter(Sender: TObject);
begin
  AjustarGrid;
end;

function TFRTallas_s.SelectedFieldName: string;
begin
  case TCPrecios.TabIndex of
...
procedure TFRTallas_s.ATraspasaLineaOrdenProduccionExecute(Sender: TObject);
var
  ID_A_M_C : variant;
  ID_S : variant;
begin
  ...
  begin
     ID_A_M_C := dm.QMver_detalles_gr_sd['ID_A_M_C'];
     ID_S := dm.QMver_detalles_gr_sd['ID_S'];
  end;
  if dm.QMver_detalles_gr_sd_pv['ID_A_M_C'] <> null then
  begin
     ID_A_M_C := dm.QMver_detalles_gr_sd_pv['ID_A_M_C'];
     if (dm.QMver_detalles_gr_sd_pv['ID_S'] > '') then
        ID_S := dm.QMver_detalles_gr_sd_pv['ID_S']
     else
        ID_S := dm.QMver_detalles_gr_sd['ID_S'];
  end;
  if (ID_A_M_C <> null) then
     TFMTraspasoPedidosAProduccionTyCKri.Create(Self).MuestraAMC(ID_A_M_C, ID_S);
end;

procedure TFRTallas_s.ATraspasaPedidoAOrdenProduccionExecute(Sender: TObject);
var
  i : integer;
begin
  if (dm.QMver_detalles_gr_sd_pv['ID_S'] > '') then
     i := dm.QMver_detalles_gr_sd_pv['ID_S']
  else
     i := dm.QMver_detalles_gr_sd['ID_S'];
  DM.TraspasaPedidoAOrdenProd(i);
end;

procedure TFRTallas_s.ATraspasaAgrupPedidoAOrdenProduccionExecute(Sender: TObject);
var
  i : integer;
begin
  if (dm.QMver_detalles_gr_sd_pv['ID_S'] > '') then
     ...
     i := dm.QMver_detalles_gr_sd['ID_S'];
  DM.TraspasaAgrPedidoAOrdenProd(i);
end;

procedure TFRTallas_s.ANuevoArticuloExecute(Sender: TObject);
begin
  FMain.EjecutaAccion(FMain.AModelosTallas);
  FMModelosTallas.Posiciona(dm.QMver_detalles_gr_sdREF_PROVEEDOR.AsString);
end;

procedure TFRTallas_s.ASeleccionarKitExecute(Sender: TObject);
var
  Kit : string;
  IdGT : integer;
  Cantidad : double;
...
procedure TFRTallas_s.AInsercionMasivaModeloExecute(Sender: TObject);
var
  RefProveedor : string;
begin
