...
{
  Modificado SelectSQL de QSPStock_Minimo para que use el proceso Z (SFG_JLA)
}

unit ZUDMRStockMinimo;
...
type
  TZDMRStockMinimo = class(TDataModule)
     ...
     procedure ZDMRStockMinimoCreate(Sender: TObject);
     procedure ZDMRStockMinimoDestroy(Sender: TObject);
  ...
  public
     { Public declarations }
     procedure CalculaInforme(Canal: integer; LCan, LAlm: TStrings; aAlmacen, aAlmacen2: string; aArticuloDesde, aArticuloHasta: string; aFamiliaDesde, aFamiliaHasta: string; StockVirtual: boolean; Modo: integer; aCanalIni, aCanalFin: integer);
  end;

var
  ZDMRStockMinimo : TZDMRStockMinimo;
...
procedure TZDMRStockMinimo.ZDMRStockMinimoCreate(Sender: TObject);
begin
...
procedure TZDMRStockMinimo.ZDMRStockMinimoDestroy(Sender: TObject);
begin
  CierraData(DMListados);
end;

procedure TZDMRStockMinimo.CalculaInforme(Canal: integer; LCan, LAlm: TStrings; aAlmacen, aAlmacen2: string; aArticuloDesde, aArticuloHasta: string; aFamiliaDesde, aFamiliaHasta: string; StockVirtual: boolean; Modo: integer; aCanalIni, aCanalFin: integer);
var
  str : string;
begin
  ...
  begin
     ...
     Params.ByName['ALMACEN2'].AsString := Almacen2;
     ...
     Params.ByName['CANAL_INI'].AsInteger := CanalIni;
     Params.ByName['CANAL_FIN'].AsInteger := CanalFin;
     Open;
  end;
  ...
  DMListados.Imprimir(9004, 0, Modo, '', _('Listado de Stock bajo minimos'), frStockMinimo, nil, nil, nil);
end;

procedure TZDMRStockMinimo.frStockMinimoEnterRect(Memo: TStringList; View: TfrView);
begin
  if (Memo.Count > 0) then
     DMListados.BeforePrint(Memo, View);
end;

procedure TZDMRStockMinimo.frStockMinimoGetValue(const ParName: string; var ParValue: variant);
begin
  ...
  if ParName = 'Almacen2' then
     ParValue := Almacen2;
  if ParName = 'Z_Almacen2' then
     ParValue := Almacen2;
  if ParName = 'AlmacenTitulo' then
     ParValue := DameTituloAlmacen(Almacen);
  if ParName = 'Almacen2Titulo' then
     ParValue := DameTituloAlmacen(Almacen2);
...
procedure TZDMRStockMinimo.ActualizarRangoSelec(LCan, LAlm: TStrings; Canal: smallint; const Alm: string);
var
  i : integer;
begin
...
procedure TZDMRStockMinimo.frxStockMinimoBeforePrint(Sender: TfrxReportComponent);
begin
...
procedure TZDMRStockMinimo.frxStockMinimoGetValue(const VarName: string; var Value: variant);
begin
