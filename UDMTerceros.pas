...
type
  TDMTerceros = class(TDataModule)
     ...
     QMTercerosDIRECCION_LITERAL: TBlobField;
     QMDireccionesDIRECCION_LITERAL: TBlobField;
     xContactosDireccion: TFIBDataSetRO;
     DSxContactosDireccion: TDataSource;
     xContactosDireccionTERCERO: TIntegerField;
     xContactosDireccionCONTACTO: TIntegerField;
     xContactosDireccionNOMBRE: TFIBStringField;
     xContactosDireccionAPELLIDOS: TFIBStringField;
     xContactosDireccionMOVIL: TFIBStringField;
     xContactosDireccionTELEFONO: TFIBStringField;
     xContactosDireccionDIRECCION: TIntegerField;
  ...
  public
     ...
     procedure BuscaDatosDireccionCP(Pais, CodigoPostal: string; var Region, Provincia, Poblacion: string);
     procedure BuscaDatosDireccionPoblacion(Pais, Poblacion: string; var Region, Provincia: string);
     function ExistenDatosDireccionPais(Pais: string): boolean;
     ...
     procedure ActualizaCasillasDteDesdeCsv;
     procedure ActualizaCoordenadas(Tercero, Direccion: integer; Lat, Lon: double);
  end;

var
  DMTerceros : TDMTerceros;
  UnaVez : byte = 0;

implementation

uses UDMMain, UUtiles, UDameDato, uFBusca, UEntorno, UFMTerceros, DateUtils, UDMLstTerceros, UFormGest, UDMListados, UParam,
  UFSendCorreo;
...
procedure TDMTerceros.DMTercerosCreate(Sender: TObject);
begin
...
procedure TDMTerceros.DMTercerosDestroy(Sender: TObject);
begin
  SQLBase.Free;   // Destruimos la sentencia SQL base
end;

procedure TDMTerceros.QMTercerosNewRecord(DataSet: TDataSet);
begin
  ...
  QMTercerosDIR_CLASE.AsInteger := 1; // Ponemos por defecto el tipo Calle
  QMTercerosDIR_LOCALIDAD.AsString := '99999999';
  if ((REntorno.Pais = 'CHL') or (REntorno.Pais = 'BOL')) then
     QMTercerosPAIS_DIRECCION.AsString := REntorno.Pais;
  QMTercerosCOMO_NOS_CONOCIERON.AsInteger := 0;
end;

procedure TDMTerceros.Graba(DataSet: TDataSet);
begin
  FMTerceros.TraspasosActivos(1);
...
procedure TDMTerceros.QMContactosNewRecord(DataSet: TDataSet);
begin
...
procedure TDMTerceros.QMTercerosBeforePost(DataSet: TDataSet);
var
  MensajeError : string;
  Valido : boolean;
begin
  ...
  begin
     QMTercerosID_LOCAL.AsInteger := 1;
     QMTercerosDIR_LOCALIDAD.AsString := '99999999';
  end;
...
procedure TDMTerceros.QMContactosBeforePost(DataSet: TDataSet);
begin
...
procedure TDMTerceros.QMBancosNewRecord(DataSet: TDataSet);
begin
...
function TDMTerceros.CodigosAuxiliares(Modo: smallint): integer;
begin
...
procedure TDMTerceros.QMBancosBeforePost(DataSet: TDataSet);
begin
  if (DataSet.State = dsInsert) then
...
procedure TDMTerceros.QMDireccionesNewRecord(DataSet: TDataSet);
begin
...
procedure TDMTerceros.QMDireccionesDIR_LOCALIDADChange(Sender: TField);
begin
  if (QMDireccionesPAIS_DIRECCION.AsString <> 'CHL') then
  begin
     with xLoc_D do
     begin
        Close;
...
procedure TDMTerceros.QMDireccionesBeforePost(DataSet: TDataSet);
begin
  if (DataSet.State = dsInsert) then
  begin
...
procedure TDMTerceros.QMBancosENTIDADChange(Sender: TField);
begin
...
procedure TDMTerceros.QMTercerosDIR_LOCALIDADChange(Sender: TField);
begin
  if (REntorno.Pais <> 'CHL') then
  begin
     // Refresco datos cuadno estoy editando/insertando
...
procedure TDMTerceros.QMTercerosDIR_TIPOChange(Sender: TField);
begin
  // Refresco datos cuadno estoy editando/insertando
  if (Sender.DataSet.State in [dsEdit, dsInsert]) then
  begin
...
procedure TDMTerceros.QMDireccionesDIR_TIPOChange(Sender: TField);
begin
  // Refresco datos cuadno estoy editando/insertando
  if (Sender.DataSet.State in [dsEdit, dsInsert]) then
  begin
...
procedure TDMTerceros.QMTercerosAfterOpen(DataSet: TDataSet);
begin
  xRazones.Open;
  xLoc_T.Open;
  xTipDir_T.Open;
  ...
  xValidacionDoc.Open;
  if (not QMContactos.Active) then
     QMContactos.Open;
end;

procedure TDMTerceros.BusquedaCompleja;
begin
  TFBusca.Create(Self).SeleccionaBusqueda(QMTerceros, '00000');
end;

procedure TDMTerceros.QueEs(Tercero: integer; var Cliente, Proveedor, Acreedor, Agente, Empleado, Potencial, Crm: boolean);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMTerceros.QMTercerosNOMBRE_COMERCIALChange(Sender: TField);
begin
  if QMTerceros.State <> dsEdit then
     QMTercerosNOMBRE_R_SOCIAL.AsString := QMTercerosNOMBRE_COMERCIAL.AsString;
end;

procedure TDMTerceros.QMTercerosTIPO_RAZONChange(Sender: TField);
begin
  // Refresco datos cuadno estoy editando/insertando
  if (Sender.DataSet.State in [dsEdit, dsInsert]) then
  begin
...
procedure TDMTerceros.AbrirFicha;
begin
  if (not xLoc_T.Active) then
     xLoc_T.Open;
...
procedure TDMTerceros.CerrarFicha;
begin
  xLoc_T.Close;
  xTipDir_T.Close;
  xRazones.Close;
...
procedure TDMTerceros.AbrirDirecciones;
begin
  QMDirecciones.Open;
end;

procedure TDMTerceros.CerrarDirecciones;
begin
  QMDirecciones.Close;
end;

procedure TDMTerceros.AbrirContactos;
begin
  QMContactos.Open;
end;

procedure TDMTerceros.CerrarContactos;
begin
  QMContactos.Close;
end;

procedure TDMTerceros.AbrirCuentas;
begin
  QMBancos.Open;
end;

procedure TDMTerceros.CerrarCuentas;
begin
  QMBancos.Close;
end;

procedure TDMTerceros.AbrirEmail;
begin
  xEmails.Close;
...
procedure TDMTerceros.CerrarEmail;
begin
  xEmails.Close;
end;

procedure TDMTerceros.InsertarTercero;
begin
  with QMTerceros do
  begin
     Close;
...
procedure TDMTerceros.FiltraTercero(Filtro: string);
begin
  with QMTerceros do
  begin
     Close;
...
procedure TDMTerceros.QMTercerosBeforeEdit(DataSet: TDataSet);
begin
  nombreold := QMTercerosNOMBRE_R_SOCIAL.AsString;
  nifold := QMTercerosNIF.AsString;
end;

procedure TDMTerceros.RefrescaNombreCuentas;
var
...
procedure TDMTerceros.RefrescaContactoCRM;
var
...
procedure TDMTerceros.CheckDuplicateNif;
var
  Duplicado : boolean;
  Continuar : boolean;
begin
...
procedure TDMTerceros.DireccionDefecto;
begin
...
procedure TDMTerceros.ResDireccionDefecto;
begin
  if (HayDireccion) then
     if confirma then
     begin
...
function TDMTerceros.HayDireccion;
begin
  Result := (QMDireccionesDIRECCION.AsInteger <> 0);
end;

procedure TDMTerceros.QMDireccionesBeforeDelete(DataSet: TDataSet);
begin
...
procedure TDMTerceros.ActualizaDirecciones;
var
  Temporal : TBookmark;
begin
...
procedure TDMTerceros.QMDireccionesAfterOpen(DataSet: TDataSet);
begin
  xLoc_D.Open;
  xTipDir_D.Open;
  xContactosDireccion.Open;
end;

procedure TDMTerceros.VerificaNifKri;
var
  Seguir : integer;
  s : string;
...
procedure TDMTerceros.QMTercerosAfterScroll(DataSet: TDataSet);
begin
...
procedure TDMTerceros.QMContactosAfterScroll(DataSet: TDataSet);
begin
  // FMTerceros.CreaArbolContactos;
end;

procedure TDMTerceros.QMContactosAfterOpen(DataSet: TDataSet);
begin
  FMTerceros.CreaArbolContactos;
end;

procedure TDMTerceros.PosicionaContacto(Contacto: integer);
begin
  with QMContactos do
  begin
     First;
...
procedure TDMTerceros.GenerarFirma;
var
...
procedure TDMTerceros.CreaTerceros;
var
...
procedure TDMTerceros.RenombraTerceros;
var
  a, n, d, b, l, z : TStrings;
  r : integer;
  Tercero : integer;
...
procedure TDMTerceros.ReemplazaCarateres;
var
  TituloForm : string;
...
begin
  ...
  begin
     ...
     begin
        ...
        begin
           ...
           FMTerceros.Caption := QMTercerosNOMBRE_COMERCIAL.AsString + QMTercerosNOMBRE_R_SOCIAL.AsString;
  ...
  FMTerceros.Caption := TituloForm;
end;

procedure TDMTerceros.ImprimirCartaSEPA(Modo: integer; MedianteClienteCorreo: boolean; MuestraCuerpo: boolean = False);
var
...
procedure TDMTerceros.ImprimirCartaLOPD(Modo: integer; MedianteClienteCorreo: boolean; MuestraCuerpo: boolean = False);
var
  CuerpoMail : TStrings;
  Archivo : string;
begin
...
procedure TDMTerceros.CalculaIBAN;
var
  TituloForm : string;
  Faltan : integer;
begin
  ...
  begin
     ...
     Faltan := RecordCount;
     First;
     FMTerceros.TSCuentas.Show;

     while not EOF do
     begin
        FMTerceros.Caption := 'Faltan: ' + IntToStr(Faltan);
        with QMBancos do
        begin
           while not EOF do
           begin
...
procedure TDMTerceros.QMDireccionesBeforeClose(DataSet: TDataSet);
begin
  xLoc_D.Close;
  xTipDir_D.Close;
  xContactosDireccion.Close;
end;

procedure TDMTerceros.QMBancosAfterOpen(DataSet: TDataSet);
begin
  xBancos.Open;
end;

procedure TDMTerceros.QMBancosBeforeClose(DataSet: TDataSet);
begin
  xBancos.Close;
end;

procedure TDMTerceros.QMTercerosPAIS_TERCEROChange(Sender: TField);
begin
...
function TDMTerceros.VerificaExisteEnTercero(Tercero: integer; Tipo: string): boolean;
var
  Cliente, Proveedor, Acreedor, Agente, Empleado, Potencial, Crm : boolean;
begin
  QueEs(QMTercerosTERCERO.AsInteger, Cliente, Proveedor, Acreedor, Agente, Empleado, Potencial, Crm);
...
procedure TDMTerceros.BuscaDatosDireccionCP(Pais, CodigoPostal: string; var Region, Provincia, Poblacion: string);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'EXECUTE PROCEDURE DAME_DATOS_DIRECCION_COD_POSTAL(:PAIS, :CODIGO_POSTAL)';
        Params.ByName['PAIS'].AsString := Pais;
        Params.ByName['CODIGO_POSTAL'].AsString := CodigoPostal;
        ExecQuery;
        Region := Trim(FieldByName['REGION'].AsString);
        Provincia := Trim(FieldByName['PROVINCIA'].AsString);
        Poblacion := Trim(FieldByName['POBLACION'].AsString);
        FreeHandle;
     finally
        Free;
     end;
...
function TDMTerceros.ExistenDatosDireccionPais(Pais: string): boolean;
var
  Region, Provincia : boolean;
  Nivel : integer;
begin
  Result := False;

  Nivel := DMMain.DameNivelDireccionPais(Pais);
  ...
  begin
     ...
     begin
        try
           Close;
           DataBase := DMMain.DataBase;
           SQL.Text := 'SELECT ID_REGION FROM SYS_REGIONES WHERE PAIS = :PAIS AND REGION <> :REGION';
           Params.ByName['PAIS'].AsString := Pais;
           Params.ByName['REGION'].AsString := 'ZZZ';
           ExecQuery;
           Region := (FieldByName['ID_REGION'].AsInteger <> 0);
           FreeHandle;
        finally
           Free;
        end;
  ...
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'SELECT ID_PROVINCIA FROM SYS_PROVINCIAS WHERE PAIS=:PAIS AND PROVINCIA <> :PROVINCIA';
        Params.ByName['PAIS'].AsString := Pais;
        Params.ByName['PROVINCIA'].AsString := '99';
        ExecQuery;
        Provincia := (FieldByName['ID_PROVINCIA'].AsInteger <> 0);
        FreeHandle;
     finally
        Free;
     end;
  ...
  if (Nivel = 4) then
     Result := (Region and Provincia);

  if (Nivel = 3) then
     Result := Provincia;
end;

procedure TDMTerceros.QMDireccionesAfterPost(DataSet: TDataSet);
begin
  Graba(DataSet);
...
procedure TDMTerceros.QMTercerosBeforeClose(DataSet: TDataSet);
begin
  xRazones.Close;
  xLoc_T.Close;
  xTipDir_T.Close;
  ...
  if (QMContactos.Active) then
     QMContactos.Close;
end;

procedure TDMTerceros.BuscaDatosDireccionPoblacion(Pais, Poblacion: string; var Region, Provincia: string);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        Close;
        DataBase := DMMain.DataBase;
        SQL.Text := 'EXECUTE PROCEDURE DAME_DATOS_DIRECCION_POBLACION(:PAIS, :POBLACION)';
        Params.ByName['PAIS'].AsString := Pais;
...
procedure TDMTerceros.TransportistaSegunCP(CP: string);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
...
procedure TDMTerceros.CreaUsuarioWeb(Tercero, Contacto, Direccion: integer; Nombre, email: string);
var
...
procedure TDMTerceros.FiltraUnTercero(Tercero: integer);
begin
  with QMTerceros do
  begin
     Close;
...
procedure TDMTerceros.QuitaFiltros;
begin
  with QMTerceros do
  begin
     Close;
...
procedure TDMTerceros.ActualizaCasillasDteDesdeCsv;

  procedure ActualizaCasillaTercero(Rut, Email: string);
  begin
     // Limpio dato y evito pasarme de tamaño de campo.

     Rut := Trim(Rut);
     if (Length(Rut) > 20) then
        Rut := Trim(Copy(Rut, 1, 20));

     Email := Trim(Email);
     if (Length(Email) > 100) then
        Email := Trim(Copy(Email, 1, 100));
...
var
  i, Campo, fin : integer;
  Archivo, Rut, EMail, Linea : string;
  DS : TFIBDataSet;
  WndList : Pointer;
  tsRUT : TStringList;
  tsFichero : TStringList;
begin
  inherited;

  // Seleccionamos el fichero
  Archivo := '';
  if MyOpenDialog(Archivo, 'CSV,ALL', '', 'DMTerceros-ActualizaCasillasDteDesdeCsv') then
  begin
     tsRUT := TStringList.Create;
     tsFichero := TStringList.Create;
     FSendCorreo := TFSendCorreo.Create(Self);

     // Deshabilito el resto de ventanas
     WndList := DisableTaskWindows(FSendCorreo.Handle);
     try
        // Cargo fichero en memoria
        TSFichero.LoadFromFile(Archivo);

        with FSendCorreo do
        begin
           Titulo(_('Actualizando casillas DTE ...'));
           MuestraBarraProgreso(TSFichero.Count);
           Show;
        end;
        ...
        try
           ...
           begin
              ...
              try
                 ...
                 SelectSQL.Add('SELECT NIF FROM SYS_TERCEROS WHERE TRIM(NIF) > '''' ORDER BY NIF');
                 Open;
                 First;
                 while not EOF do
                 begin
                    tsRUT.Add(FieldByName('NIF').AsString);
                    Next;
                 end;
                 Close;
                 Transaction.Commit;
        ...
        // Recorro lineas del fichero
        i := 0;
        while (i < TSFichero.Count - 1) do
        begin
           Linea := TSFichero[i];

           // Obtengo los campos de RUT e Email de la linea
           Campo := 0;
           Rut := '';
           Email := '';
           repeat
              fin := Pos(';', Linea);
              case Campo of
                 0: Rut := Trim(Copy(Linea, 0, fin - 1));
                 4: Email := Trim(Copy(Linea, 0, fin - 1));
              end;
              System.Delete(Linea, 1, fin);
              Inc(Campo);
           until not ((fin > 0) and (Campo <= 4));

           if (Rut > '') then
           begin
              // Verificamos si existe el Rut en terceros y actualizamos email para ese tercero
              if (tsRUT.IndexOf(Rut) >= 0) then
              begin
                 FSendCorreo.Texto(Format(_('Registro : %d') + #13#10 + _('RUT: %s') + #13#10 + _('Email: %s'), [i, Rut, Email]));
                 ActualizaCasillaTercero(Rut, Email);
              end;
           end
           else
           if (i mod 10 = 0) then
           begin
              // Refresco progreso cada 10 registros para acelerar proceso
              FSendCorreo.Texto(Format(_('Registro : %d') + #13#10 + _('RUT: %s') + #13#10 + _('Email: %s'), [i, Rut, Email]));
           end;
...
procedure TDMTerceros.ActualizaCoordenadas(Tercero, Direccion: integer; Lat, Lon: double);
begin
  with THYFIBQuery.Create(nil) do
  begin
     try
        ...
        SQL.Add(' UPDATE SYS_TERCEROS_DIRECCIONES ');
        SQL.Add(' SET ');
        SQL.Add(' LATITUD = :LATITUD, ');
        SQL.Add(' LONGITUD = :LONGITUD ');
        SQL.Add(' WHERE ');
        SQL.Add(' TERCERO = :TERCERO AND ');
        SQL.Add(' DIRECCION = :DIRECCION ');
        Params.ByName['TERCERO'].AsInteger := Tercero;
        Params.ByName['LATITUD'].AsFloat := Lat;
        Params.ByName['LONGITUD'].AsFloat := Lon;
        Params.ByName['DIRECCION'].AsInteger := Direccion;
        ExecQuery;
        FreeHandle;
     finally
